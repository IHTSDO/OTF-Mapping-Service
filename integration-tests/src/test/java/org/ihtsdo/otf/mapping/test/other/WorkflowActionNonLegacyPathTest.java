...
public class WorkflowActionNonLegacyPathTest {
  ...
  public static void init() throws Exception {
    ...
    for (Concept c : contentService.getConcepts().getIterable()) {
      ...
      for (TreePosition tp : contentService.getTreePositions(
          c.getTerminologyId(), c.getTerminology(), c.getTerminologyVersion())
          .getTreePositions()) {
    ...
    contentService.computeTreePositions("sourceTerminology",
        "sourceTerminologyVersion", "0", "1");
    ...
    mapProject.setRuleBased(false); // NOTE: If this is changed to true, must
  ...
  public void testNormalWorkflowWithConflict() throws Exception {
    ...
    // assign specialist 1
    workflowService.processWorkflowAction(mapProject, concept, specialist,
        null, WorkflowAction.ASSIGN_FROM_SCRATCH);

    // retrieve record - expect owner set with status NEW
    for (MapRecord mr : workflowService.getMapRecordsForProjectAndConcept(
        mapProject.getId(), concept.getTerminologyId()).getMapRecords()) {
    ...
    // save for later
    workflowService.processWorkflowAction(mapProject, concept, specialist,
        specRecord, WorkflowAction.SAVE_FOR_LATER);
    assertTrue(workflowService.getMapRecord(specRecord.getId())
        .getWorkflowStatus().equals(WorkflowStatus.EDITING_IN_PROGRESS));

    // finish editing
    workflowService.processWorkflowAction(mapProject, concept, specialist,
        specRecord, WorkflowAction.FINISH_EDITING);
    ...
    // save for later
    workflowService.processWorkflowAction(mapProject, concept, specialist2,
        specRecord2, WorkflowAction.SAVE_FOR_LATER);
    ...
    workflowService.processWorkflowAction(mapProject, concept, specialist2,
        specRecord2, WorkflowAction.FINISH_EDITING);
    Thread.sleep(1000);
    ...
    assertTrue(specRecord.getWorkflowStatus().equals(
    ...
    assertTrue(workflowService.getMapRecord(specRecord.getId())
        .getWorkflowStatus().equals(WorkflowStatus.CONFLICT_IN_PROGRESS));

    // finish editing
    workflowService.processWorkflowAction(mapProject, concept, lead,
        leadRecord, WorkflowAction.FINISH_EDITING);
    assertTrue(workflowService.getMapRecord(leadRecord.getId())
    ...
    assertNull(workflowService.getTrackingRecordForMapProjectAndConcept(
        mapProject, concept));
    ...
    assertTrue(workflowService.getMapRecord(leadRecordId).getWorkflowStatus()
