...
public class WorkflowActionNonLegacyPathTest {
	...
	public static void init() throws Exception {
		...
		for (MapUser mu : mappingService.getMapUsers().getIterable())
			if (!mu.getUserName().equals("guest") && !mu.getUserName().equals("loader")
		...
		mapProject.setRuleBased(false); // NOTE: If this is changed to true, must set rules in map entry test cases
	...
	public void testNormalWorkflowWithConflict() throws Exception {
		...
		assertTrue(mappingService.getMapRecord(specRecord.getId()).getWorkflowStatus()
				.equals(WorkflowStatus.EDITING_IN_PROGRESS));

		// finish editing
		workflowService.processWorkflowAction(mapProject, concept, specialist, specRecord,
				WorkflowAction.FINISH_EDITING);
		assertTrue(mappingService.getMapRecord(specRecordId).getWorkflowStatus()
		...
		workflowService.processWorkflowAction(mapProject, concept, specialist2, specRecord2,
				WorkflowAction.SAVE_FOR_LATER);
		assertTrue(mappingService.getMapRecord(specRecordId2).getWorkflowStatus()
				.equals(WorkflowStatus.EDITING_IN_PROGRESS));
		...
		workflowService.processWorkflowAction(mapProject, concept, specialist2, specRecord2,
				WorkflowAction.FINISH_EDITING);
		Thread.sleep(1000);
		MapRecord test = mappingService.getMapRecord(specRecordId);
		assertTrue(mappingService.getMapRecord(specRecordId).getWorkflowStatus()
		...
		assertTrue(specRecord.getWorkflowStatus().equals(WorkflowStatus.CONFLICT_NEW));
		...
		// save for later
		workflowService.processWorkflowAction(mapProject, concept, lead, leadRecord,
				WorkflowAction.SAVE_FOR_LATER);
		assertTrue(mappingService.getMapRecord(specRecord.getId()).getWorkflowStatus()
		...
		workflowService.processWorkflowAction(mapProject, concept, lead, leadRecord,
				WorkflowAction.FINISH_EDITING);
		assertTrue(mappingService.getMapRecord(leadRecord.getId()).getWorkflowStatus()
		...
		assertNull(mappingService.getMapRecord(specRecordId));
		assertNull(mappingService.getMapRecord(specRecordId2));
		
		// expect lead record to be PUBLISHED
		assertTrue(mappingService.getMapRecord(leadRecordId).getWorkflowStatus().equals(WorkflowStatus.PUBLISHED));
	}
