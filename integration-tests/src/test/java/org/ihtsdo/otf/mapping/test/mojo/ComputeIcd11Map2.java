...
public class ComputeIcd11Map2 {
	...
	final Set<String> advicesToInclude = new HashSet<>(Arrays.asList(new String[] {
			"POSSIBLE REQUIREMENT FOR AN EXTERNAL CAUSE CODE", "THIS CODE IS NOT TO BE USED IN THE PRIMARY POSITION",
			"THIS IS AN EXTERNAL CAUSE CODE FOR USE IN A SECONDARY POSITION" }));

	final Set<String> advicesToExclude = new HashSet<>(Arrays.asList(new String[] {
			"FIFTH CHARACTER REQUIRED TO FURTHER SPECIFY THE SITE", "MAP IS CONTEXT DEPENDENT FOR GENDER",
			"MAP OF SOURCE CONCEPT IS CONTEXT DEPENDENT", "MAPPED FOLLOWING IHTSDO GUIDANCE",
			"MAPPED FOLLOWING WHO GUIDANCE", "MAPPING GUIDANCE FROM WHO IS AMBIGUOUS",
			"POSSIBLE REQUIREMENT FOR ADDITIONAL CODE TO FULLY DESCRIBE DISEASE OR CONDITION",
			"POSSIBLE REQUIREMENT FOR CAUSATIVE AGENT CODE", "POSSIBLE REQUIREMENT FOR MORPHOLOGY CODE",
			"POSSIBLE REQUIREMENT FOR PLACE OF OCCURRENCE", "SOURCE SNOMED CONCEPT IS AMBIGUOUS",
	...
	public void test() throws Exception {
		...
		try {
			...
			Logger.getLogger(getClass()).info("   SORT (leaf) = " + sctScope.get(sctScope.size() - 2));
			Logger.getLogger(getClass()).info("   SORT (leaf) = " + sctScope.get(sctScope.size() - 1));

			// Configure writers
			final PrintWriter mapOut = new PrintWriter(new FileWriter(new File(icd11Dir, "icd11Map.txt")));
			final PrintWriter notesOut = new PrintWriter(new FileWriter(new File(icd11Dir, "icd11MapNotes.txt")));
			final PrintWriter statsOut = new PrintWriter(new FileWriter(new File(icd11Dir, "icd11MapStats.txt")));
			...
			for (final String sctid : sctScope) {
				...
				sctidCt++;
				Logger.getLogger(getClass()).info("------------------------------");
				Logger.getLogger(getClass()).info("SCTID = " + sctid + " " + sctConcepts.get(sctid));
			...
			final Category category : Category.values()) {
				Logger.getLogger(getClass()).info(category.toString() + " = " + stats[category.ordinal()]);
				statsOut.println(category.toString() + " = " + stats[category.ordinal()]);
			}
			for (
			...
			mapOut.print("id\teffectiveTime\tactive\tmoduleId\trefsetId\treferencedComponentId\tmapGroup\t"
	...
	public RuleDetails[] gatherMapEvidence(String sctid, int index) {
		...
		final IcdMap map10 = (icd10Map.get(sctid).size() >= index) ? icd10Map.get(sctid).get(index - 1) : null;
		final String icd10Code = map10 != null ? map10.getMapTarget() : null;
		final String icd10Name = (icd10Code == null || icd10Code.equals("")) ? "No Mapping"
				: icd10Concepts.get(icd10Code);
		Logger.getLogger(getClass()).info("  ICD10 Map = " + icd10Code + ", " + icd10Name + ", " + map10);

		//
		// RULE0 - "no mapping"
		// - bypass this rule for "symptoms" in snomed
		rules[0] = new RuleDetails(0);
		if (icd10Code != null && icd10Code.equals("") && !sctConcepts.get(sctid).toLowerCase().contains("symptom")) {
		...
		if (map10 != null && icd10To11.containsKey(map10.getMapTarget())) {
			...
			for (final WhoMap map : icd10To11.get(map10.getMapTarget())) {
				...
				double modifier = 1.0;
				if (sctConcepts.get(sctid).startsWith("Neoplasm of ") && !sctConcepts.get(sctid).contains("behavior")
						&& !sctConcepts.get(sctid).contains("behaviour") && icd11Concepts.get(code) != null) {
				...
				} else if (map.getRelation().equals("PotentialIntersect") || map.getRelation().equals("InterSects")) {
			...
			if (!sctConcepts.get(sctid).toLowerCase().contains(" with ") && !equivalent
					&& rules[2].getScoreMap().size() > 5) {
				for (final String code : new HashSet<>(rules[2].getScoreMap().keySet())) {
		...
		if (sctIcd11Matches.containsKey(sctid)) {
			...
			for (final Score score : sctIcd11Matches.get(sctid)) {
				...
				if (!icd11Scope.contains(code)) {
					if (!code.endsWith("unspecified") && icd11Scope.contains(code + "/morbidity/unspecified")) {
						code = code + "/morbidity/unspecified";
					} else if (!code.endsWith("other") && icd11Scope.contains(code + "/morbidity/other")) {
		...
		if (sctChdPar.containsKey(sctid)) {
			...
			for (final String par : sctChdPar.get(sctid)) {
				if (!icd11Map.containsKey(par)) {
					...
					Logger.getLogger(getClass()).warn(sctid + " without PAR map " + par + " " + sctConcepts.get(par));
					continue;
				}
		...
		for (final RuleDetails rule : new RuleDetails[] { rules[1], rules[2], rules[3], rules[4] }) {
			// Boost all scores if "unspecified" and ends with Z/other
			for (final String code : rule.scoreMap.keySet()) {
				if (code != null && sctConcepts.get(sctid).toLowerCase().contains("unspecified")
			...
			for (final String code : rule.scoreMap.keySet()) {
				...
				if (code != null && icd11Concepts.get(parCode) != null
						&& sctConcepts.get(sctid).toLowerCase().contains(icd11Concepts.get(parCode))
						&& code.endsWith("other")) {
					System.out.println("YYYY: " + sctid + ", " + sctConcepts.get(sctid) + " = " + code);
					rule.scoreMap.put(code, rule.scoreMap.get(code) * 1.5);
					rules[5].addScore(code, 1d);
			...
			for (final String parCode : rule.scoreMap.keySet()) {
				for (final String chdCode : rule.scoreMap.keySet()) {
					if (chdCode != null && parCode != null && icd11ParChd.get(parCode) != null
	 ...
	 *             the exception
	 */
	public RuleScores scoreMapEvidence(String sctid, IcdMap map10, RuleDetails[] rules) throws Exception {
		...
		// Compute NC Score
		final double ncScore = rules[0].getNcScore() + rules[1].getNcScore() + rules[2].getNcScore()
				+ rules[3].getNcScore() + rules[4].getNcScore();
		...
		if (ncScore > 3.0) {
			...
			map11.setMapAdvice("MAP SOURCE CONCEPT CANNOT BE CLASSIFIED WITH AVAILABLE DATA");
		...
		// Compute scores from the RuleDetails
			...
			for (final RuleDetails rule : new RuleDetails[] { rules[1], rules[2], rules[3] }) {
				for (final String code : rule.getScoreMap().keySet()) {
			...
			// Calculate rule scores - ignore RULE0
			// Boost rule4
			for (final RuleDetails rule : new RuleDetails[] { rules[4] }) {
				for (final String code : rule.getScoreMap().keySet()) {
			...
			for (final String word : requiredWords.keySet()) {
				...
				if (sctConcepts.get(sctid).toLowerCase().contains(word)) {
					for (final String key : new HashSet<>(candidates.keySet())) {
						if (icd11Concepts.containsKey(key)
								&& !icd11Concepts.get(key).toLowerCase().contains(requiredWords.get(word))) {
				...
				if (!sctConcepts.get(sctid).toLowerCase().contains(word)) {
					for (final String key : new HashSet<>(candidates.keySet())) {
						if (icd11Concepts.containsKey(key) && icd11Concepts.get(key).toLowerCase().contains(word)) {
			...
			for (final String key : new HashSet<>(candidates.keySet())) {
				if (icd11Concepts.containsKey(key)
						&& icd11Concepts.get(key).toLowerCase().contains("sustained full remission")) {
					candidates.put(key, candidates.get(key) * 1.01d);
					rules[5].addScore(key, 1d);
					rules[5].appendType(key, "BOOST-DOWN (sustained full remission) 1.01");
				}
			}
			...
			if (sctBodyPartMap.containsKey(sctid)) {
				...
				for (final String key : new HashSet<>(candidates.keySet())) {
					...
					if (icd11BodyPartMap.containsKey(key) && bodyPartIcd11Map.containsKey(bodyPart)
							&& bodyPartIcd11Map.get(bodyPart).contains(key)) {
						candidates.put(key, candidates.get(key) * 1.5d);
						rules[5].addScore(key, 1d);
						rules[5].appendType(key, "BOOST (" + sctBodyPartMap.get(sctid) + ") 1.5");
					}
				}
			}
			...
			// AND ICD11 does, down-boost
				...
				for (final String key : new HashSet<>(candidates.keySet())) {
					if (icd11BodyPartMap.containsKey(key)) {
						...
						candidates.put(key, candidates.get(key) * 0.75d);
						rules[5].addScore(key, 1d);
						rules[5].appendType(key, "BOOST-DOWN (" + icd11BodyPartMap.get(key) + ") 0.75");
					}
				}
			}
			...
			if (maxScore == 0.0 && ncScore == 0.0) {
				...
				if (icd10Map.containsKey(sctid) && icd10Map.get(sctid).iterator().next().getMapTarget().equals("")) {
					category = Category.NO_MAP;
					targetId = "";
					candidates.put(targetId, ncScore);
					map11.setMapAdvice("MAP SOURCE CONCEPT CANNOT BE CLASSIFIED WITH AVAILABLE DATA");
		...
		if (!targetId.equals("") && !icd11Concepts.containsKey(targetId)) {
			throw new Exception("Invalid target id, not in icd11Concepts.txt - " + targetId);
		}
		...
		noteSb.append(": ").append(" (").append(candidates.get(targetId)).append(") ").append(targetId).append(" ")
				.append(icd11Concepts.get(targetId)).append("\n");
		boolean first = false;
		final List<String> sortedCandidateKeys = new ArrayList<>(candidates.keySet());
		...
		final String other : sortedCandidateKeys) {
			...
			noteSb.append("(").append(candidates.get(other)).append(") ").append(other).append(" ");
	 ...
	 * @return the list
	 */
	public Category applyOverrides(String sctid, List<IcdMap> mapList, Category category, RuleScores primaryScores) {
		...
		final Category[] categoryWrapper = new Category[] { category };
		final Map<String, Double> candidates = primaryScores.getCandidates();

		boolean override = false;
		if (!override) {
			override = applyXhNeoplasmRule(sctid, icd10Map.get(sctid), mapList, noteSb, candidates, categoryWrapper);
		}
		if (!override) {
			override = applyXaNeoplasmRule(sctid, icd10Map.get(sctid), mapList, noteSb, candidates, categoryWrapper);
		}
		if (!override) {
			override = applyXtAdditionRule(sctid, icd10Map.get(sctid), mapList, noteSb, candidates, categoryWrapper);
		}
		if (!override) {
			override = applyWithRule(sctid, icd10Map.get(sctid), mapList, noteSb, candidates, categoryWrapper);
		}
		if (!override) {
			override = applyDiabetesRule(sctid, icd10Map.get(sctid), mapList, noteSb, candidates, categoryWrapper);
		}
		...
		if (icd11Map.get(sctid).size() == 1 && category == Category.HIGH
				&& (sctName.contains(" with ") || sctName.contains(" due to ")
						|| sctName.toLowerCase().contains("and/or") || sctName.contains(" AND "))) {
		...
		if (sctConcepts.get(sctid).toLowerCase().contains("sickle cell") && categoryWrapper[0] == Category.HIGH) {
	 ...
	 *            the primary scores
	 */
	public void applyAdvices(String sctid, List<IcdMap> mapList, RuleScores primaryScores) {
		...
		for (final IcdMap map11 : mapList) {
			...
			// target, clear advice
			if (i == 0) {
				if (!primaryScores.getRuleDetails()[2].getScoreMap().containsKey(map11.getMapTarget())) {
			...
			for (final String adviceToRemove : advicesToExclude) {
				map11.setMapAdvice(getWithoutAdvice(map11.getMapAdvice(), adviceToRemove));
			}
			...
			if (sctAncDesc.get("75478009").contains(sctid) || sctAncDesc.get("125605004").contains(sctid)
					|| sctAncDesc.get("55342001").contains(sctid)) {
				map11.setMapAdvice(getWithoutAdvice(map11.getMapAdvice(), "MAPPED FOLLOWING WHO GUIDANCE"));
			}
	 ...
	 */
	private boolean applyXhNeoplasmRule(String sctid, List<IcdMap> icd10Map, List<IcdMap> icd11MapOrig,
			StringBuilder noteSb, Map<String, Double> candidates, Category[] category) {
		...
		boolean override = false;
		double score = 0;
		if (sctAncDesc.get("55342001").contains(sctid) && !sctAncDesc.get("118599009").contains(sctid)
				&& !sctAncDesc.get("118601006").contains(sctid) && icd10Map.size() == 1 && icd11MapOrig.size() == 1) {
			final String targetId = icd11MapOrig.iterator().next().getMapTarget();
			final String targetCode = icd11Concepts.get(targetId);
			if (targetCode != null && targetCode.startsWith("XH") && candidates.get(targetId) >= 2.0) {
		...
		if (override) {
			...
			stemMap.setMapAdvice(getWithoutAdvice(stemMap.getMapAdvice(), "POSSIBLE REQUIREMENT FOR MORPHOLOGY CODE"));
			origMap.setMapAdvice(getWithoutAdvice(origMap.getMapAdvice(), "POSSIBLE REQUIREMENT FOR MORPHOLOGY CODE"));
			noteSb.append("\n\nOVERRIDE HIGH: (" + score + "): XH Neoplasm Rule (added stem code)\n");
	 ...
	 * @return true, if successful
	 */
	private boolean applyXtAdditionRule(String sctid, List<IcdMap> icd10Map, List<IcdMap> icd11MapOrig,
			StringBuilder noteSb, Map<String, Double> candidates, Category[] category) {
		...
		boolean override = false;
		double score = 0;
		if ((sctAncDesc.get("40733004").contains(sctid) || sctAncDesc.get("128139000").contains(sctid))
			...
			if (targetCode != null && candidates.get(targetId) >= 0.4) {
				...
				for (final String key : xtConcepts.keySet()) {
					if (icd11Index.get(targetId) != null && sctName.startsWith(key)
							&& icd11Index.get(targetId).contains(sctName.replace(key + " ", ""))) {
		...
		if (override) {
			...
			icd11Map.add(xtMap);
			noteSb.append(
					"\nOVERRIDE " + category[0].toString() + ": (" + score + "): XT Addition Rule - " + xtName + "\n");
	 ...
	 * @return true, if successful
	 */
	private boolean applyWithRule(String sctid, List<IcdMap> icd10Map, List<IcdMap> icd11MapOrig, StringBuilder noteSb,
		...
		if ((sctConcepts.get(sctid).toLowerCase().contains(" with ")
				|| sctConcepts.get(sctid).toLowerCase().contains(" associated with ")) && icd10Map.size() == 1
				&& icd11MapOrig.size() == 1) {
		...
		if (override) {
			...
			noteSb.append("\nOVERRIDE " + category[0].toString() + ": (" + score + "): WITH Rule - " + withName + "\n");
	 ...
	 * @return true, if successful
	 */
	private boolean applyDiabetesRule(String sctid, List<IcdMap> icd10Map, List<IcdMap> icd11MapOrig,
			StringBuilder noteSb, Map<String, Double> candidates, Category[] category) {
	...
	private void fixAdvice(IcdMap map11, String targetId) {
		if (map11.getMapAdvice().startsWith("ALWAYS")) {
			map11.setMapAdvice(map11.getMapAdvice().replaceAll("ALWAYS ([^\\s])+", "ALWAYS " + targetId));
		} else {
			map11.setMapAdvice("ALWAYS " + targetId);
		}
	 ...
	 * @return true, if successful
	 */
	private boolean applyXaNeoplasmRule(String sctid, List<IcdMap> icd10Map, List<IcdMap> icd11MapOrig,
			StringBuilder noteSb, Map<String, Double> candidates, Category[] category) {
		...
		boolean override = false;
		double score = 0;
		if (sctAncDesc.get("55342001").contains(sctid) && icd10Map.size() == 1 && icd11MapOrig.size() == 1) {
			final String targetId = icd11MapOrig.iterator().next().getMapTarget();
			final String targetCode = icd11Concepts.get(targetId);
			if (targetCode != null && targetCode.startsWith("2") && candidates.get(targetId) >= 1.5) {
				score += candidates.get(targetId);
				for (final String otherTargetId : candidates.keySet()) {
					final String otherTargetCode = icd11Concepts.get(otherTargetId);
	...
	private void loadDataResources(String icd11Dir) throws Exception {
		...
		List<String> lines = FileUtils.readLines(new File(icd11Dir, "sctScopeDesc.txt"), "UTF-8");
		int ct = 0;
		int skipCt = 0;
		...
		for (final String line : lines) {
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(fields[5] + " => " + icd11MapEdits.get(fields[5]));
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
		...
		lines = FileUtils.readLines(new File(icd11Dir, "icd11Concepts.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
			...
			if (fields[1].startsWith("XA")) {
				if (fields[1].lastIndexOf(" ") == (fields[1].indexOf(":") + 1)) {
					Logger.getLogger(getClass())
							.info("  XA = " + fields[1].substring(fields[1].indexOf(":") + 2).toLowerCase());
					xtConcepts.put(fields[1].substring(fields[1].indexOf(":") + 2).toLowerCase(), fields[0]);
				}
			}
			ct++;
		}
		...
		lines = FileUtils.readLines(new File(icd11Dir, "LinearizationIndexTabulation-Morbidity-en.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\t");
			// Strip - http://id.who.int/icd/entity
			final String code = fields[0].replace("http://id.who.int/icd/entity/", "");
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(code + " => " + fields[4].replaceAll("\"", "").toLowerCase());
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
		...
		lines = FileUtils.readLines(new File(icd11Dir, "icd10Concepts.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
		...
		for (final String line : lines) {
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(fields[5] + " => " + icd10Map.get(fields[5]));
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
		...
		lines = FileUtils.readLines(new File(icd11Dir, "sctScopeAncDesc.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(fields[0] + " => DESC " + sctAncDesc.get(fields[0]));
				Logger.getLogger(getClass()).debug(fields[1] + " => ANC " + sctDescAnc.get(fields[1]));
			}
		...
		lines = FileUtils.readLines(new File(icd11Dir, "sctScopeBodyPartMap.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
		...
		lines = FileUtils.readLines(new File(icd11Dir, "icd11BodyPartMap.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
		...
		for (final String line : lines) {
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(fields[0] + " => CHD " + icd11ParChd.get(fields[0]));
			}
		...
		Logger.getLogger(getClass()).info("  Remove icd11 scope concepts that have children");
		ct = 0;
		for (final String par : icd11ParChd.keySet()) {
			// keep things of a specified length
			final String code = icd11Concepts.get(par);
			if (code.contains(":") && code.substring(0, code.indexOf(':') - 1).length() >= 6) {
				continue;
			}
		...
		lines = FileUtils.readLines(new File(icd11Dir, "sctIcdRollup.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(fields[0] + " => " + sctIcd10Rollup.get(fields[0]));
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
		...
		lines = FileUtils.readLines(new File(icd11Dir, "sctIcd11Equivalence.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\t");
			...
			if (icd11Scope.contains(code)) {
				...
				if (ct < sampleCt) {
					Logger.getLogger(getClass()).debug(sctid + " => " + sctIcd11Equivalence.get(sctid) + ", "
		...
		lines = FileUtils.readLines(new File(icd11Dir, "sctIcd11Matches.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\|");
			final Score score = new Score(line);
			final String code = fields[0].replaceAll("http\\:\\/\\/id.who.int\\/icd\\/entity\\/", "");
			if (!sctIcd11Matches.containsKey(fields[0])) {
				sctIcd11Matches.put(code, new ArrayList<Score>());
			}
			sctIcd11Matches.get(code).add(score);
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(code + " => " + sctIcd11Matches.get(code));
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
		...
		//
		lines = FileUtils.readLines(new File(icd11Dir, "10To11MapToMultipleCategories.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\t");
			...
			if (!icd10Scope.contains(map.getCode())) {
				skipCt++;
				Logger.getLogger(getClass()).debug("SKIP (scope " + map.getCode() + ") ");// +
																							// line);
				continue;
			}
		...
		lines = FileUtils.readLines(new File(icd11Dir, "11To10MapToOneCategory.txt"), "UTF-8");
		ct = 0;
		skipCt = 0;
		for (final String line : lines) {
			final String[] fields = line.split("\\t");
			...
			if (!map.getTargetCode().equals("No Mapping") && !icd10Scope.contains(map.getTargetCode())) {
				skipCt++;
				Logger.getLogger(getClass()).debug("SKIP (scope " + map.getTargetCode() + ") ");// +
																								// line);
				continue;
			}
			...
			if (ct < sampleCt) {
				Logger.getLogger(getClass()).debug(code + " => " + icd11To10Reverse.get(code));
			}
			ct++;
		}
		Logger.getLogger(getClass()).info("   ct = " + ct);
	...
	class WhoMap {
		...
		public WhoMap(String line) throws Exception {
			...
			if (fields.length == 17 || fields.length == 18 || fields.length == 15) {
				...
				name = fields[4];
				targetKind = fields[5];
				targetCode = fields[7].replaceAll("http\\:\\/\\/id.who.int\\/icd\\/entity\\/", "");
				...
				chapter = fields[2];
				name = fields[3];
				targetCode = fields[4].replaceAll("http\\:\\/\\/id.who.int\\/icd\\/entity\\/", "");
				...
				throw new Exception("Unexpected number of fields - " + fields.length + ", " + line);
			}
		}
		...
		@Override
		public String toString() {
			return "WhoMap [kind=" + kind + ", depthInKind=" + depthInKind + ", code=" + code + ", chapter=" + chapter
					+ ", name=" + name + ", targetKind=" + targetKind + ", targetDepthInKind=" + targetDepthInKind
					+ ", targetCode=" + targetCode + ", targetChapter=" + targetChapter + ", chapterMatch="
					+ chapterMatch + ", targetName=" + targetName + ", relation=" + relation + ", distance=" + distance
					+ ", issueType=" + issueType + ", matchType=" + matchType + ", stated=" + stated + "]";
		}
	...
	class IcdMap {
		...
		@Override
		public String toString() {
			return "IcdMap [mapGroup=" + mapGroup + ", mapPriority=" + mapPriority + ", mapRule=" + mapRule
					+ ", mapAdvice=" + mapAdvice + ", mapTarget=" + mapTarget + ", mapCategoryId=" + mapCategoryId
					+ "]";
		}
		...
		public String toNote(Map<String, String> icd11Concepts) {
			...
			for (final String key : scoreMap.keySet()) {
				...
				if (typeMap.containsKey(key)) {
					sb.append(typeMap.get(key)).append(" (").append(scoreMap.get(key)).append(") ").append(key)
							.append(" ");
				} else {
