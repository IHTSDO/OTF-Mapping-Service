...
call %MVN_HOME%/bin/mvn -Drun.config=dev -Dhibernate.hbm2ddl.auto=create install 1> mvn.log
...
call %MVN_HOME%/bin/mvn -PSNOMEDCT -Drun.config=dev install 1> mvn.log
IF %ERRORLEVEL% NEQ 0 (set error=1
goto trailer)
del /Q mvn.log
...
cd %OTF_MAPPING_HOME%/admin/loader
call %MVN_HOME%/bin/mvn -PICPC -Drun.config=dev install 1> mvn.log
IF %ERRORLEVEL% NEQ 0 (set error=1
goto trailer)
del /Q mvn.log
...
cd %OTF_MAPPING_HOME%/admin/loader
call %MVN_HOME%/bin/mvn -PICD9CM -Drun.config=dev install 1> mvn.log
IF %ERRORLEVEL% NEQ 0 (set error=1
goto trailer)
del /Q mvn.log
