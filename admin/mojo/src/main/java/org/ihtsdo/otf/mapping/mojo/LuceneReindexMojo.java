...
@Execute(goal = "reindex", phase = LifecyclePhase.PACKAGE)
	...
	public void execute() throws MojoFailureException {
		try {
			...
			String authToken = service
					.authenticate(properties.getProperty("admin.user"), properties.getProperty("admin.password"))
					.getAuthToken();
			service.close();
			...
			if (!serverRunning) {
				...
				// AdminServiceRestImpl adminService = new
				// AdminServiceRestImpl();
				...
				// invoke the client
				Logger.getLogger(getClass()).info("Content Client - lucene reindex " + indexedObjects);

				final Client client = ClientBuilder.newClient();
				final WebTarget target = client.target(properties.getProperty("base.url") + "/admin/reindex");
				final Response response = target.request(MediaType.APPLICATION_JSON).header("Authorization", authToken)
