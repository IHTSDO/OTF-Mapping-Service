...
public class IndexXmlToLuceneMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			...
			// TODO: put this into "lucene" directory of baseDir, not hibernate
			// setting!
			String indexBaseDir = ConfigUtility.getConfigProperties().getProperty("hibernate.search.default.indexBase");
			...
			dictionaryFile = new File(dictionaryDir, dictionaryFile).getAbsolutePath();
			...
			PerFieldAnalyzerWrapper analyzer = new PerFieldAnalyzerWrapper(new StandardAnalyzer(Version.LUCENE_36),
					fieldAnalyzers);

			// Open index writer
			IndexWriterConfig config = new IndexWriterConfig(Version.LUCENE_36, analyzer);
			config.setOpenMode(OpenMode.CREATE);
			writer = new IndexWriter(new SimpleFSDirectory(new File(outputDir)), config);
			...
			if (!file.exists()) {
				throw new MojoFailureException(
						"Specified index.viewer.ICD10.xml directory does not exist: " + inputFile);
			}
	...
	class LocalHandler extends DefaultHandler {
		...
		private void addDictionaryWords() {
			...
			while (st.hasMoreTokens()) {
				...
				if (!StandardAnalyzer.STOP_WORDS_SET.contains(token) && token.length() > 2) {
