  ...
  public void execute() throws MojoFailureException {
    ...
    try {
      ...
      for (MapProject mpr : mappingService.getMapProjects().getMapProjects()) {
        ...
        StringBuffer mprMapAdministrators = new StringBuffer();
        for (MapUser ma : mpr.getMapAdministrators()) {
          mprMapAdministrators.append(ma.getUserName()).append(",");
        }
        if (mprMapAdministrators.length() > 1)
          mprMapAdministrators.deleteCharAt(mprMapAdministrators.length() - 1);
        ...
        projectsWriter.write(mpr.getName() + "\t" + mpr.getRefSetId() + "\t"
            ...
            + "\t" + mprMapSpecialists + "\t" + mprMapAdministrators + "\t" + mpr.isScopeDescendantsFlag()
