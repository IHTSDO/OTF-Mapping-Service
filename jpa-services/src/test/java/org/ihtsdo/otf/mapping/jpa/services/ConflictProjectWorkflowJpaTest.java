...
public class ConflictProjectWorkflowJpaTest {
	...
	public void testStandardProgressionWithConflict() throws Exception {
		...
		MapRecordList records = mappingService.getMapRecordsForConcept(concept.getTerminologyId());
		if (records.getCount() == 0)
			...
			if (!records.getIterable().iterator().next().getWorkflowStatus().equals(WorkflowStatus.READY_FOR_PUBLICATION))
  ...
  public void retrieveAssignedWork() throws Exception {
		...
		SearchResultList assignedWork1 = workflowService.findAssignedWork(
				mapProject, specialist1, null, null);
		SearchResultList assignedWork2 = workflowService.findAssignedWork(
				mapProject, specialist2, null, null);
		SearchResultList assignedConflicts = workflowService
				.findAssignedConflicts(mapProject, lead, null, null);

		// try to retrieve specialist 1's record, if it exists
		if (assignedWork1.getCount() != 0) {
			specialistRecord1 = mappingService.getMapRecord(assignedWork1
					.getIterable().iterator().next().getId());
		}

		// try to retrieve specialist 1's record, if it exists
		if (assignedWork2.getCount() != 0) {
			specialistRecord2 = mappingService.getMapRecord(assignedWork2
					.getIterable().iterator().next().getId());
		}

		// try to retrieve specialist 1's record, if it exists
		if (assignedConflicts.getCount() != 0) {
			leadRecord = mappingService.getMapRecord(assignedConflicts
					.getIterable().iterator().next().getId());
		}
	}
	...
	public static void destroyAll() throws Exception {
		...
		for (TrackingRecord trackingRecord : workflowService
				.getTrackingRecords().getIterable()) {
