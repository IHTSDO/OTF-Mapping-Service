...
public class MapRecordRf2ComplexMapLoaderAlgorithm extends RootServiceJpa
	...
	public void compute() throws Exception {
	    ...
	    try {
	      ...
	      if (userName == null) {
	        Logger.getLogger(getClass()).info("No user specified, defaulting to user 'loader'");
	      }
	      ...
	      if (recordFlag) {
	        ...
	        for (final String refSetId : refSetIds) {
	          ...
	          Logger.getLogger(getClass()).info("  Refset " + refSetId + " count = " + ct);
	      ...
	      e.printStackTrace();
	      throw new Exception("Loading of RF2 Complex Maps failed.",
	          e);
	    } finally {
	      try {
	        mappingService.close();
