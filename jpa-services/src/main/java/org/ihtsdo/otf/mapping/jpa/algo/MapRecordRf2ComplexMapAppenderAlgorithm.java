...
public class MapRecordRf2ComplexMapAppenderAlgorithm extends RootServiceJpa
  ...
  public MapRecordRf2ComplexMapAppenderAlgorithm(String inputFile,
      String refsetId, String workflowStatus) throws Exception {
    super();
  ...
  public void compute() throws Exception {
    ...
    try {
      ...
      for (final String refSetId : refSetIds) {
        ...
        // Identify which members' source concepts already have existing records
        // which will need to be updated

        final List<ComplexMapRefSetMember> newComplexMembers =
            new ArrayList<>();
        final List<ComplexMapRefSetMember> updateComplexMembers =
            new ArrayList<>();

        MapProject mapProject =
            mappingService.getMapProjectForRefSetId(refSetId);
        MapRecordList mapRecordsForProject =
            mappingService.getMapRecordsForMapProject(mapProject.getId());

        final Set<String> existingMapConceptIds = new HashSet<>();
        for (MapRecord mapRecord : mapRecordsForProject.getMapRecords()) {
          existingMapConceptIds.add(mapRecord.getConceptId());
        }

        for (ComplexMapRefSetMember complexMapRefsetMember : complexMembers) {
          if (existingMapConceptIds.contains(
              complexMapRefsetMember.getConcept().getTerminologyId())) {
            updateComplexMembers.add(complexMapRefsetMember);
          } else {
            newComplexMembers.add(complexMapRefsetMember);
          }
        }
        ...
        // For brand-new concepts, create maps from scratch
        mappingService.createMapRecordsForMapProject(
            mapProjectMap.get(refSetId).getId(), loaderUser, newComplexMembers,
            status);

        // For concepts that already have an existing map, append to that map
        mappingService.appendEntriesToMapRecordsForProject(
            mapProjectMap.get(refSetId).getId(), loaderUser,
            updateComplexMembers, status);
      }
