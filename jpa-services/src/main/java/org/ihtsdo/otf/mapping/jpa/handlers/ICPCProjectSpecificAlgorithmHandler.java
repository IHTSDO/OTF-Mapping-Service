 ...
 */
public class ICPCProjectSpecificAlgorithmHandler extends DefaultProjectSpecificAlgorithmHandler {
	...
	/**
	 * Computes the map relation for the SNOMEDCT->ICD10 map project.
	 * Based solely on whether an entry has a TRUE rule or not.
	 * No advices are computed for this project.
	 */
	@Override
	public MapRelation computeMapRelation(MapRecord mapRecord, MapEntry mapEntry) {
		...
		if (mapEntry.getRule().equals("TRUE")) {
			...
			for (MapRelation relation : mapProject.getMapRelations()) {
				if (relation.getTerminologyId().equals("447637006")) return relation;
			}
		...
		// if entry has a target and not TRUE rule	
			...
			// retrieve the relations by terminology id
			for (MapRelation relation : mapProject.getMapRelations()) {
				if (relation.getTerminologyId().equals("447639009")) return relation;
			}
