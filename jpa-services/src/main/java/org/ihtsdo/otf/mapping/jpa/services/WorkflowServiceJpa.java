 ...
 */
public class WorkflowServiceJpa extends MappingServiceJpa implements
    WorkflowService {

  /** The workflow path handlers. */
  private static Map<String, WorkflowPathHandler> workflowHandlerMap =
      new HashMap<>();
  ...
  static {
    try {
      ...
      final String key = "workflow.path.handler";
      for (final String handlerName : config.getProperty(key).split(",")) {
        if (handlerName.isEmpty())
          continue;
        // Add handlers to map
        WorkflowPathHandler handlerService =
            ConfigUtility.newStandardHandlerInstanceWithConfiguration(key,
                handlerName, WorkflowPathHandler.class);
    ...
    switch (mapProject.getWorkflowType()) {
      case CONFLICT_PROJECT:
        ...
        if (mapProject.isTeamBased() && mapUser.getTeam() != null
          ...
          for (final MapUser user : getMapUsersForTeam(mapUser.getTeam())
              .getMapUsers()) {
  ...
  public void processWorkflowAction(MapProject mapProject, Concept concept,
    ...
    if (trackingRecord == null) {
      ...
      if (workflowAction.equals(WorkflowAction.CREATE_QA_RECORD)) {
        handler = getWorkflowPathHandler("QA_PATH");
        trackingRecord.setWorkflowPath(WorkflowPath.QA_PATH);
      }

      // otherwise, use Fix Error Path
      else {
        handler = getWorkflowPathHandler("FIX_ERROR_PATH");
    ...
    // otherwise, instantiate based on tracking record
      ...
      Logger.getLogger(WorkflowServiceJpa.class).info(
          "  Tracking Record: " + trackingRecord.toString());
    ...
    // process the workflow action
    mapRecords =
        handler.processWorkflowAction(trackingRecord, workflowAction,
    ...
    getWorkflowStatusForTrackingRecord(trackingRecord).equals(
  ...
  public SearchResultList findAvailableWork(MapProject mapProject,
    ...
    switch (mapProject.getWorkflowType()) {
      ...
      default:
        throw new Exception(
            "findAvailableWork: could not determine workflow type for project");
  ...
  public SearchResultList findAssignedWork(MapProject mapProject,
    ...
    switch (mapProject.getWorkflowType()) {
      ...
      default:
        throw new Exception(
            "findAssignedWork: could not determine workflow type for project");
  ...
  public WorkflowPathHandler getWorkflowPathHandler(String name) {
    return workflowHandlerMap.get(name);
