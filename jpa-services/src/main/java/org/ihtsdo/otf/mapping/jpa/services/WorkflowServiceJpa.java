 ...
 */
public class WorkflowServiceJpa extends MappingServiceJpa implements WorkflowService {
	...
	public SearchResultList findAvailableWork(MapProject mapProject, final MapUser mapUser, String query,
		...
		case CONFLICT_PROJECT:
			...
			if (mapProject.isTeamBased() && mapUser.getTeam() != null && !mapUser.getTeam().isEmpty()) {
				...
				for (final MapUser user : getMapUsersForTeam(mapUser.getTeam()).getMapUsers()) {
	...
	public void createQAWork(Report report) throws Exception {
		...
		for (final String conceptId : conceptIds) {
			...
			final Concept concept = contentService.getConcept(conceptId, mapProject.getSourceTerminology(),
					mapProject.getSourceTerminologyVersion());

			final MapRecordList recordList = getMapRecordsForProjectAndConcept(mapProject.getId(), conceptId);
			// lazy initialize
	...
	public void processWorkflowAction(MapProject mapProject, Concept concept, MapUser mapUser, MapRecord mapRecord,
		...
		// otherwise, instantiate based on tracking record
			...
			Logger.getLogger(WorkflowServiceJpa.class).info("  Tracking Record: " + trackingRecord.toString());
		...
		// process the workflow action
		mapRecords = handler.processWorkflowAction(trackingRecord, workflowAction, mapProject, mapUser, mapRecords,
				mapRecord);
	...
	public void computeWorkflow(MapProject mapProject) throws Exception {
		...
		SearchResultList conceptsInScope = findConceptsInScope(mapProject.getId(), null);
	...
	public List<String> computeWorkflowStatusErrors(MapProject mapProject) throws Exception {
		...
		final ContentService contentService = new ContentServiceJpa();
		for (final MapRecord mapRecord : getMapRecordsForMapProject(mapProject.getId()).getMapRecords()) {
	...
	public void computeUntrackedMapRecords(MapProject mapProject) throws Exception {
		...
		final MapRecordList mapRecordsInProject = getMapRecordsForMapProject(mapProject.getId());
	...
	public MapRecordList getOriginMapRecordsForConflict(Long mapRecordId) throws Exception {
		...
		Logger.getLogger(MappingServiceJpa.class)
				.info("getRecordsInConflict with record id = " + mapRecordId.toString());
		...
		if (mapRecord == null) {
			throw new Exception(
					"getRecordsInConflict: Could not find map record with id = " + mapRecordId.toString() + "!");
		}
		...
		} else if ((mapProject.getWorkflowType().equals(WorkflowType.CONFLICT_PROJECT)
		...
		(mapProject.getWorkflowType().equals(WorkflowType.REVIEW_PROJECT) && mapRecord.getOriginIds().size() > 2)) {
			...
			TrackingRecord tr = workflowService.getTrackingRecordForMapProjectAndConcept(mapProject,
			...
			if (tr.getWorkflowPath().equals(WorkflowPath.REVIEW_PROJECT_PATH)
				...
				for (final Long originId : mapRecord.getOriginIds()) {
					...
					try {
						MapRecord mr = getMapRecord(mapRecord.getOriginIds().iterator().next());
				...
				throw new Exception("Could not retrieve exactly one origin id for REVIEW_PROJECT path");
			}

		} else {
			throw new Exception("Invalid map record passed to conflict origins routine");
		}
