...
public class WorkflowServiceJpa implements WorkflowService {
  ...
  public void removeWorkflowTrackingRecord(Long workflowTrackingRecordId)
    ...
    if (getTransactionPerOperation()) {
      EntityTransaction tx = manager.getTransaction();
      tx.begin();
      WorkflowTrackingRecord ma =
          manager.find(WorkflowTrackingRecordJpa.class,
              workflowTrackingRecordId);
   ...
   * 
   * @see
   * org.ihtsdo.otf.mapping.services.WorkflowService#getAvailableWork(org.ihtsdo
  ...
  public SearchResultList findAvailableWork(MapProject mapProject,
    ...
    if (pfsParameter != null) {
      ...
      toIndex =
          pfsParameter.getMaxResults() == -1 ? availableWork.size() : Math.min(
              availableWork.size(), startIndex + pfsParameter.getMaxResults());
    }
   ...
   * 
   * @see
   * org.ihtsdo.otf.mapping.services.WorkflowService#getAvailableConflicts(org
  ...
  public SearchResultList findAvailableConflicts(MapProject mapProject,
    ...
    if (pfsParameter != null) {
      startIndex =
          pfsParameter.getStartIndex() == -1 ? 0 : pfsParameter.getStartIndex();
      toIndex =
          pfsParameter.getMaxResults() == -1 ? availableConflicts.size() : Math
              .min(availableConflicts.size(),
  ...
  public SearchResultList findAssignedWork(MapProject mapProject,
    ...
    for (WorkflowTrackingRecord trackingRecord : getWorkflowTrackingRecordsForMapProject(mapProject)) {
      ...
      // cycle over map records to find the NEW or EDITING_IN_PROGRESS record
    ...
    if (pfsParameter != null) {
      startIndex =
          pfsParameter.getStartIndex() == -1 ? 0 : pfsParameter.getStartIndex();
      toIndex =
          pfsParameter.getMaxResults() == -1 ? assignedWork.size() : Math.min(
              assignedWork.size(), startIndex + pfsParameter.getMaxResults());
    }
   ...
   * 
   * @see
   * org.ihtsdo.otf.mapping.services.WorkflowService#getAssignedConflicts(org
   * .ihtsdo.otf.mapping.model.MapProject, org.ihtsdo.otf.mapping.model.MapUser)
   */
  @Override
  public SearchResultList findAssignedConflicts(MapProject mapProject,
    ...
    for (WorkflowTrackingRecord trackingRecord : getWorkflowTrackingRecordsForMapProject(mapProject)) {
      ...
      // cycle over map records to find the CONFLICT_DETECTED record assigned to
      // this user
      for (MapRecord mapRecord : trackingRecord.getMapRecords()) {
        if (mapRecord.getOwner().equals(mapUser)
            && mapRecord.getWorkflowStatus().compareTo(
    ...
    if (pfsParameter != null) {
      startIndex =
          pfsParameter.getStartIndex() == -1 ? 0 : pfsParameter.getStartIndex();
      toIndex =
          pfsParameter.getMaxResults() == -1 ? assignedConflicts.size() : Math
              .min(assignedConflicts.size(),
                  startIndex + pfsParameter.getMaxResults());
    }
  ...
  public SearchResultList findAvailableConsensusWork(MapProject mapProject,
    ...
    if (pfsParameter != null) {
      startIndex =
          pfsParameter.getStartIndex() == -1 ? 0 : pfsParameter.getStartIndex();
      toIndex =
          Math.min(availableConsensus.size(),
              startIndex + pfsParameter.getMaxResults());
    }
  ...
  public void processWorkflowAction(MapProject mapProject, Concept concept,
    ...
    // force a read of map records and users and detach the record for passing
    // into project algorithm handler
    // Rationale: the algorithm handler is not expected to perform any actions
    // requiring services
    ...
    switch (workflowAction) {
      case ASSIGN_FROM_INITIAL_RECORD:
        ...
        trackingRecord =
            algorithmHandler.assignFromInitialRecord(trackingRecord, mapRecord,
                mapUser);
      ...
      case ASSIGN_FROM_SCRATCH:
        ...
        trackingRecord =
            algorithmHandler
                .assignFromScratch(trackingRecord, concept, mapUser);
      ...
      case UNASSIGN:
        ...
        // expect existing (pre-computed) workflow tracking record to exist with
        // this user assigned
        if (trackingRecord == null)
          throw new Exception(
      ...
      case SAVE_FOR_LATER:
        ...
        // record
        if (!trackingRecord.getAssignedUsers().contains(mapUser))
          throw new Exception("SAVE_FOR_LATER - User not assigned to record");
      ...
      case FINISH_EDITING:
        ...
        // perform the action
        trackingRecord =
            algorithmHandler.finishEditing(trackingRecord, mapUser);
  ...
  public void synchronizeWorkflowTrackingRecord(
    ...
    List<MapRecord> oldRecords =
        new ArrayList<>(oldTrackingRecord.getMapRecords());
    for (int i = 0; i < oldRecords.size(); i++) {

      MapRecord mapRecord = oldRecords.get(i);

      // if this map record is not in the new tracking record, it should be
      // deleted
    ...
    List<MapRecord> newRecords =
        new ArrayList<>(newTrackingRecord.getMapRecords());
    for (int i = 0; i < newRecords.size(); i++) {
      ...
      if (getMapRecordInWorkflowTrackingRecord(oldTrackingRecord,
        ...
        Logger.getLogger(WorkflowServiceJpa.class).info(
            "       --> Added record " + mapRecord.getId().toString());
  ...
  public void computeWorkflow(MapProject mapProject) throws Exception {
    ...
    SearchResultList unmappedConceptsInScope =
        mappingService.findUnmappedConceptsInScope(mapProject.getId());
    Logger.getLogger(WorkflowServiceJpa.class).info(
    ...
    for (SearchResult sr : unmappedConceptsInScope.getSearchResults()) {
      ...
      trackingRecord.setWorkflowPath(WorkflowPath.NON_LEGACY_PATH); // Make this
                                                                    // a project
                                                                    // specific
                                                                    // call
                                                                    // later
      ...
      // get the tree positions for this concept and set the sort key to the
      // first retrieved
      SearchResultList treePositionsList =
      ...
      if (treePositionsList.getSearchResults().size() == 0) {
        ...
        trackingRecord.setSortKey(treePositionsList.getSearchResults().get(0)
            .getValue());
      }
