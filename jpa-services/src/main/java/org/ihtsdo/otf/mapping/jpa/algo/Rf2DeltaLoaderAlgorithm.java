...
public class Rf2DeltaLoaderAlgorithm extends RootServiceJpa
	...
	private void setup() throws Exception {
		...
		if (lastPublicationDate == null) {
			...
			final javax.persistence.Query query = contentService
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!fields[0].equals("id")) {
				...
				if (conceptCache.containsKey(fields[4])) {
					...
					concept = contentService.getConcept(
							existingConceptCache.get(fields[4]).getId());
				} else {
					// retrieve concept
				...
				if (concept != null) {
					...
					newDescription
							.setCaseSignificanceId(Long.valueOf(fields[8]));
					...
					// later)
						...
						newDescription.setEffectiveTime(
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!fields[0].equals("id")) {
				...
				// If language refset entry is changed, update it
					...
					recomputePnConceptIds
							.add(description.getConcept().getTerminologyId());
					contentService.updateLanguageRefSetMember(
							newLanguageRefSetMember);
					cacheLanguageRefSetMember(newLanguageRefSetMember);
					objectsUpdated++;
				}
				...
				// Otherwise, reset effective time (for modified check later)
					...
					newLanguageRefSetMember.setEffectiveTime(
							languageRefSetMember.getEffectiveTime());
				}
			}
		}
		...
		while ((line = reader.readLine()) != null) {
			...
			if (!fields[0].equals("id")) {
				...
				if (conceptCache.containsKey(fields[4])) {
					...
					sourceConcept = contentService.getConcept(
							existingConceptCache.get(fields[4]).getId());
				} else {
				...
				if (conceptCache.containsKey(fields[5])) {
					...
					destinationConcept = contentService.getConcept(
							existingConceptCache.get(fields[5]).getId());
				} else {
				...
				newRelationship
						.setRelationshipGroup(Integer.valueOf(fields[6])); // relationshipGroup
				newRelationship.setTypeId(Long.valueOf(fields[7])); // typeId
				newRelationship
						.setCharacteristicTypeId(Long.valueOf(fields[8])); // characteristicTypeId
				...
				// Otherwise, reset effective time (for modified check later)
					...
					if (relationship != null) {
						newRelationship.setEffectiveTime(
								relationship.getEffectiveTime());
					}
				}
	...
	/**
	 * Retires concepts that were removed from prior deltas. Find concepts in
	 * the DB that are not in the current delta and which have effective times
	 * greater than the latest release date. The latest release date is the
	...
	public void retireRemovedContent() throws Exception {
		...
		for (Concept concept : existingConceptCache.values()) {
			if (concept.getEffectiveTime().after(rf2Version)
				...
				if (proceed) {
					for (Relationship relationship : concept
							.getRelationships()) {
						if (!relationship.getEffectiveTime()
								.after(rf2Version)) {
							proceed = false;
							break;
						}
