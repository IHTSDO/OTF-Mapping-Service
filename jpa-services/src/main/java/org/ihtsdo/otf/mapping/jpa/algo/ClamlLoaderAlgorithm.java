...
import org.ihtsdo.otf.mapping.jpa.algo.helpers.ClamlMetadataHelper;
...
public class ClamlLoaderAlgorithm extends RootServiceJpa
	...
	/** The helper. */
	private ClamlMetadataHelper helper;
	...
	public ClamlLoaderAlgorithm(String terminology,
			String terminologyVersion, String inputFile) throws Exception {
	...
	public void compute() throws Exception {
		...
		helper = new ClamlMetadataHelper(terminology, terminologyVersion,
		...
		try (FileInputStream fis = new FileInputStream(file);
			...
			final Map<String, String> hierRelTypeMap = metadataService
					.getHierarchicalRelationshipTypes(terminology,
							terminologyVersion);

			final String isaRelType = hierRelTypeMap.keySet().iterator().next()
					.toString();
	...
	private class LocalHandler extends DefaultHandler {
		...
		/**
		 * This is a code => modifier map. The modifier must then be looked up
		 * in modifier map to determine the code extensions and template
		 * concepts associated with it.
		 */
		private Map<String, List<String>> classToModifierMap = new HashMap<>();

		/**
		 * This is a code => modifier map. If a code is modified but also
		 * blocked by an entry in here, do not make children from the template
		 * classes.
		 */
		private Map<String, List<String>> classToExcludedModifierMap = new HashMap<>();

		/**
		 * The rels map for holding data for relationships that will be built
		 * after all concepts are created.
		 */
		...
		/**
		 * The concept that is currently being built from the contents of a
		 * Class tag.
		 */
		...
		public void startElement(String uri, String localName, String qName,
			...
			if (qName.equalsIgnoreCase("modifier")) {
				code = attributes.getValue("code");
				classUsage = attributes.getValue("usage");
				Logger.getLogger(getClass()).info("  Encountered modifier "
						+ code + " "
						+ (classUsage == null ? "" : "(" + classUsage + ")"));
			}
			...
			if (qName.equalsIgnoreCase("modifierclass")) {
				...
				classUsage = attributes.getValue("usage");
				Logger.getLogger(getClass()).info("  Encountered modifierClass "
						+ modifierCode + " for " + modifier + " "
						+ (classUsage == null ? "" : "(" + classUsage + ")"));
			}
			...
			if (qName.equalsIgnoreCase("superclass")) {
				if (parentCode != null)
					throw new IllegalStateException(
							"Multiple SuperClass entries for " + code + " = "
									+ parentCode + ", "
			...
			// Encountered ExcludeModifier, save excluded modifier code
			// information
				...
				if (code.indexOf("-") != -1) {
					...
					for (char c = letterStart; c <= letterEnd; c++) {
						for (int i = start; i <= end; i++) {
							...
							final String code = c + padI.substring(
									padI.length() - startEnd[0].length() + 1,
									padI.length());
							Logger.getLogger(getClass())
									.info("  Class and subclasses of " + code
											+ " exclude modifier "
											+ excludeModifierCode);
							currentModifiers = new ArrayList<>();
							if (classToExcludedModifierMap.containsKey(code)) {
								currentModifiers = classToExcludedModifierMap
										.get(code);
							}
							currentModifiers.add(excludeModifierCode);
							classToExcludedModifierMap.put(code,
									currentModifiers);
						}
					}
				}
			...
			if (qName.equalsIgnoreCase("rubric")) {
				...
				Logger.getLogger(getClass()).info("  Class " + code
						+ " has rubric " + rubricKind + ", " + rubricId);
			}
			...
			if (qName.equalsIgnoreCase("reference")) {
				...
				// doesn't actually resolve to a code. We need this because it
				// is
			...
			try {
				...
				if (qName.equalsIgnoreCase("label")
					...
					if (!conceptMap.containsKey(code)) {
						...
						concept.setModuleId(new Long(conceptMap
								.get("defaultModule").getTerminologyId()));
						concept.setDefinitionStatusId(new Long(
								conceptMap.get("defaultDefinitionStatus")
										.getTerminologyId()));
					...
					Logger.getLogger(getClass())
							.info("  Add Description for class " + code + " - "
									+ rubricKind + " - "
									+ (desc.getTerm().replaceAll("\r", "")
											.replaceAll("\n", "")));
					if (conceptMap.containsKey(rubricKind))
						desc.setTypeId(new Long(
								conceptMap.get(rubricKind).getTerminologyId()));
					else {
						throw new IllegalStateException(
				...
				// concept/description
				if (qName.equalsIgnoreCase("label")
						&& tagStack.contains("modifier")) {
					// reset label characters
					labelChars = new StringBuilder();
				}

				// Encountered </Reference>, create info for later relationship
				// creation
				if (qName.equalsIgnoreCase("reference")) {
					// relationships for this concept will be added at
					// endDocument(),
					// save relevant data now in relsMap
					reference = chars.toString();
					Logger.getLogger(getClass())
							.info("  Class " + code + " has reference to "
									+ reference + " " + (referenceUsage == null
											? "" : "(" + referenceUsage + ")"));
					...
					// IF the reference tag didn't have a code attribute, use
					// the text
					// instead
					if (referenceCode == null) {
						referenceCode = reference;
					}
					String key = referenceCode + ":" + rubricId + ":"
							+ referenceUsage + ":" + reference;
					// check assumption: key is unique
					if (relsMap.containsKey(key)) {
						throw new Exception("Rels key already exists: " + key);
					}
					// because of checking assumption, will never have >1 in the
					// set
						...
						if (conceptMap.containsKey(parentCode)) {
							...
							// otherwise, make a new id
								...
								relationship.setTerminologyId(
										new Integer(relIdCounter++).toString());
							}
							relationship.setEffectiveTime(
									dateFormat.parse(effectiveTime));
							relationship.setActive(true);
							relationship.setModuleId(new Long(conceptMap
									.get("defaultModule").getTerminologyId()));
							relationship.setTerminology(terminology);
							relationship
									.setTerminologyVersion(terminologyVersion);
							relationship.setCharacteristicTypeId(new Long(
									conceptMap.get("defaultCharacteristicType")
											.getTerminologyId()));
							relationship.setModifierId(
									new Long(conceptMap.get("defaultModifier")
											.getTerminologyId()));
							relationship.setDestinationConcept(
									conceptMap.get(parentCode));
							relationship.setSourceConcept(childConcept);
							if (!conceptMap.containsKey(type))
								throw new IllegalStateException(
										"type not in conceptMap " + type);
							relationship.setTypeId(new Long(
									conceptMap.get(type).getTerminologyId()));
		...
		public void addModifierClass() throws Exception {
			...
			if (!conceptMap.containsKey(code)) {
				...
				concept.setActive(true);
				concept.setModuleId(new Long(
						conceptMap.get("defaultModule").getTerminologyId()));
				concept.setDefinitionStatusId(new Long(conceptMap
						.get("defaultDefinitionStatus").getTerminologyId()));
				concept.setTerminology(terminology);
				concept.setTerminologyVersion(terminologyVersion);
				concept.setDefaultPreferredName(labelChars.toString());
				Logger.getLogger(getClass())
						.info("  Add modifier concept "
								+ concept.getTerminologyId() + " "
								+ concept.getDefaultPreferredName());
			...
			desc.setEffectiveTime(dateFormat.parse(effectiveTime));
			desc.setActive(true);
			desc.setModuleId(new Long(
					conceptMap.get("defaultModule").getTerminologyId()));
			desc.setTerminology(terminology);
			desc.setTerminologyVersion(terminologyVersion);
			desc.setTerm(chars.toString());
			desc.setConcept(concept);
			desc.setCaseSignificanceId(new Long(conceptMap
					.get("defaultCaseSignificance").getTerminologyId()));
			desc.setLanguageCode("en");
			desc.setTypeId(
					new Long(conceptMap.get(rubricKind).getTerminologyId()));
		...
		public void modifierHelper(String codeToModify) throws Exception {
			...
			final Map<String, String> excludedModifiersToMatchedCodeMap = new HashMap<>();
			while (cmpCode.length() > 2) {
				...
				if (classToModifierMap.containsKey(cmpCode)) {
					...
					for (final String modifier : classToModifierMap
							.get(cmpCode)) {
						modifiersToMatchedCodeMap.put(modifier, codeToModify);
						Logger.getLogger(getClass()).info("      Use modifier "
								+ modifier + " for " + cmpCode);
						// If this modifier has been explicitly excluded at a
						// lower level
						// then remove it. Note: if there's an excluded modifier
						// higher up
						// it doesn't apply here because this modifier
						// explicitly overrides
						// that exclusion
						if (excludedModifiersToMatchedCodeMap
								.containsKey(modifier)
								&& isDescendantCode(
										excludedModifiersToMatchedCodeMap
												.get(modifier),
										cmpCode)) {
							Logger.getLogger(getClass())
									.info("      Exclude modifier " + modifier
											+ " for "
											+ modifiersToMatchedCodeMap
													.get(modifier)
											+ " due to "
											+ excludedModifiersToMatchedCodeMap
													.get(modifier));
							if (!overrideExclusion(codeToModify, modifier)) {
								...
								Logger.getLogger(getClass())
										.info("      Override exclude modifier "
												+ modifier + " for "
												+ codeToModify);
							}
						}
				...
				// NOTE: this can go after the earlier section because we'll
				// always
				// find an excluded modifier at a level lower than where it was
				// defined
				if (classToExcludedModifierMap.containsKey(cmpCode)) {
					for (final String modifier : classToExcludedModifierMap
						...
						excludedModifiersToMatchedCodeMap.put(modifier,
								cmpCode);
					}
				}

				cmpCode = ClamlLoaderAlgorithm.this.chdParMap
						.get(cmpCode);
				if (cmpCode == null) {
					break;
				}
			...
			Logger.getLogger(getClass())
					.info("      Final modifiers to generate classes for: "
							+ modifiersForCode);

			if (modifiersForCode.size() > 0) {

				// Loop through all modifiers identified as applying to this
				// code
				for (final String modifiedByCode : modifiersForCode) {

					// Apply 4th digit modifiers to 3 digit codes (and
					// recursively call)
					// Apply 5th digit modifiers to 4 digit codes (which have
					// length 5 due
					// to .)
					if (codeToModify.length() == 3
							&& modifiedByCode.endsWith("_4")
							|| codeToModify.length() == 5
									&& modifiedByCode.endsWith("_5")) {

						Logger.getLogger(getClass())
								.info("        Apply modifier " + modifiedByCode
										+ " to " + codeToModify);
						...
						if (modifierMap.containsKey(modifiedByCode)) {
							...
							for (final Map.Entry<String, Concept> mapEntry : modifierMap
								...
								final Concept modConcept = modifierMap
										.get(modifiedByCode)
										.get(mapEntry.getKey());

								// handle case where a _5 modifier is defined
								// with a code having
								// .*
								String childCode = null;
								if (modifiedByCode.endsWith("_5")
										&& mapEntry.getKey().startsWith("."))
									childCode = conceptMap.get(codeToModify)
											.getTerminologyId()
									...
									childCode = conceptMap.get(codeToModify)
											.getTerminologyId()
											+ mapEntry.getKey();
								createChildConcept(childCode,
										conceptMap.get(codeToModify),
										modConcept, relIdCounter++);

								// Recursively call for 5th digit modifiers on
								// generated classes
								if (codeToModify.length() == 3
										&& modifiedByCode.endsWith("_4")) {
							...
							throw new Exception("modifiedByCode not in map "
									+ modifiedByCode);
						}

					}

					// Handle case of 3 digit code with a _5 modifier without
					// any children
					else if (codeToModify.length() == 3
							&& parChildrenMap.get(codeToModify) == null
							&& modifiedByCode.endsWith("_5")) {

						final Concept conceptToModify = conceptMap
								.get(codeToModify);
						Logger.getLogger(getClass())
								.info("        Creating placeholder concept "
										+ conceptToModify.getTerminologyId()
										+ ".X");
						final Concept placeholderConcept = new ConceptJpa();
						placeholderConcept.setDefaultPreferredName(
								" - PLACEHOLDER 4th digit");

						// Recursively call for 5th digit modifiers where there
						// are no
						// child
						// concepts and the code is only 3 digits, fill in with
						// X
		...
		private boolean overrideExclusion(String code, String modifier) {
			...
			Logger.getLogger(getClass()).info("    CHECK OVERRIDE " + code
					+ ", " + cmpCode + ", " + modifier);

			final Set<String> overrideCodes = new HashSet<>(
					Arrays.asList(new String[] {
							// 4TH AND 5TH
							"V09", "V19", "V29", "V39", "V49", "V59", "V69",
							"V79", "V80", "V81", "V82", "V83", "V84", "V85",
							"V86", "V87", "V88", "V89", "V95", "V96", "V97",
							...
							"W42", "W43", "W44", "W45", "W46", "W49", "W50",
							"W51", "W52", "W53", "W54", "W55", "W56", "W57",
							"W58", "W59", "W60", "W64", "W65", "W66", "W67",
							"W68", "W69", "W70", "W73", "W74", "W75", "W76",
							"W77", "W78", "W79", "W80", "W81", "W83", "W84",
							"W85", "W86", "W87", "W88", "W89", "W90", "W91",
							"W92", "W93", "W94", "W99", "X00", "X01", "X02",
							"X03", "X04", "X05", "X06", "X08", "X09", "X10",
							...
							"X53", "X54", "X57", "X58", "X59", "Y06", "Y07",
							"Y35", "Y36", "Y40", "Y41", "Y42", "Y43", "Y44",
							"Y45", "Y46", "Y47", "Y48", "Y49", "Y50", "Y51",
							"Y52", "Y53", "Y54", "Y55", "Y56", "Y57", "Y58",
							"Y59", "Y63", "Y64", "Y65", "Y83", "Y84", "Y85",
							"Y87", "Y88", "Y89", "Y90", "Y91" }));
		...
		/**
		 * Indicates whether or not descendant code is a descendant of the
		 * ancestor code.
		 * 
		 * @param desc
		 *            the descendant code
		 * @param anc
		 *            the candidate ancestor code
		 * @return <code>true</code> if so, <code>false</code> otherwise
		 */
		private boolean isDescendantCode(String desc, String anc) {
			String currentCode = desc;
			while (ClamlLoaderAlgorithm.this.chdParMap
					.get(currentCode) != null) {
				final String parent = ClamlLoaderAlgorithm.this.chdParMap
						.get(currentCode);
		 ...
		 *             the exception
		 */
		private Concept createChildConcept(String childCode,
				Concept parentConcept, Concept modConcept, int relId)
				throws Exception {
			Logger.getLogger(getClass()).info("        Creating concept "
					+ childCode + " from " + parentConcept.getTerminologyId());
			Concept childConcept = new ConceptJpa();
			childConcept = helper.createNewActiveConcept(childCode, terminology,
					terminologyVersion,
			...
			if (!conceptUsageMap.containsKey(childConcept.getTerminologyId())
					&& conceptUsageMap
							.containsKey(parentConcept.getTerminologyId())) {
				final String usage = conceptUsageMap
						.get(parentConcept.getTerminologyId());
				final SimpleRefSetMember member = new SimpleRefSetMemberJpa();
				member.setConcept(childConcept);
				member.setActive(true);
				member.setEffectiveTime(dateFormat.parse(effectiveTime));
				member.setModuleId(new Long(
						conceptMap.get("defaultModule").getTerminologyId()));
				member.setTerminology(terminology);
				member.setTerminologyId(
						new Integer(refSetMemberCounter++).toString());
			...
			// add relationship
			helper.createIsaRelationship(parentConcept, childConcept,
					("" + relId), terminology, terminologyVersion,
					effectiveTime);
	...
	public void findVersion(String inputFile) throws Exception {
		...
		Logger.getLogger(getClass())
				.info("terminologyVersion: " + terminologyVersion);

		try {
			dateFormat.parse(effectiveTime);
			Logger.getLogger(getClass())
					.info("effectiveTime: " + effectiveTime);
		} catch (Exception e) {
			// just use today
