...
public class ClamlLoaderAlgorithm extends RootServiceJpa
  ...
  public ClamlLoaderAlgorithm(String terminology, String terminologyVersion,
  ...
  private class LocalHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
      ...
      if (qName.equalsIgnoreCase("meta")) {
        ...
        if (name != null && name.equalsIgnoreCase("toplevelsort")) {
          ...
          for (final String code : value.split(" ")) {
            log.info("  Adding root: " + code.trim());
            roots.add(code.trim());
          }
        }
        if (roots.size() == 0)
      ...
      if (qName.equalsIgnoreCase("modifier")) {
        code = attributes.getValue("code");
        classUsage = attributes.getValue("usage");
        log.info("  Encountered modifier " + code + " "
            + (classUsage == null ? "" : "(" + classUsage + ")"));
      }
      ...
      if (qName.equalsIgnoreCase("modifierclass")) {
        ...
        if (modifier.endsWith("_4") && !modifierCode.startsWith(".")) {
          log.info("  FIXING broken code, adding . to _4 code");
          modifierCode = "." + modifierCode;
        }
        if (modifier.endsWith("_5") && modifierCode.startsWith(".")) {
          log.info("  FIXING broken code, removing . from _5 code");
      ...
      if (qName.equalsIgnoreCase("modifiedby")) {
        ...
        log.info("  Class " + code + " modified by " + modifiedByCode);
      ...
      if (qName.equalsIgnoreCase("excludemodifier")) {
        ...
        log.info("  Class and subclasses of " + code + " exclude modifier "
            + excludeModifierCode);
        List<String> currentModifiers = new ArrayList<>();
            ...
            for (int i = start; i <= end; i++) {
              ...
              final String code =
                  c + padI.substring(padI.length() - startEnd[0].length() + 1,
                      padI.length());
              log.info("  Class and subclasses of " + code
      ...
      try {
        ...
        if (qName.equalsIgnoreCase("label") && tagStack.contains("class")) {
          ...
          if (conceptMap.containsKey(rubricKind))
            desc.setTypeId(
                Long.valueOf(conceptMap.get(rubricKind).getTerminologyId()));
          else {
        ...
        if (qName.equalsIgnoreCase("reference")) {
          ...
          log.info("  Class " + code + " has reference to " + reference + " "
        ...
        if (qName.equalsIgnoreCase("modifierclass")) {
          ...
          log.info("  Modifier " + modifier + " needs template class for "
              + modifierCode);
        }
        ...
        if (qName.equalsIgnoreCase("class")
          ...
          if (classUsage != null) {
            ...
            member.setModuleId(Long
                .valueOf(conceptMap.get("defaultModule").getTerminologyId()));
            member.setTerminology(terminology);
            member.setTerminologyId(
                Integer.valueOf(refSetMemberCounter++).toString());
      ...
      try {
        for (final Map.Entry<String, Set<Concept>> mapEntry : relsMap
          ...
          for (final Concept childConcept : mapEntry.getValue()) {
            log.info("  Create Relationship " + childConcept.getTerminologyId()
                + " " + type + " " + parentCode + " " + id);
            if (conceptMap.containsKey(parentCode)) {
              ...
              // otherwise, make a new id
                ...
                relationship.setTerminologyId(
                    Integer.valueOf(relIdCounter++).toString());
              }
              relationship.setEffectiveTime(dateFormat.parse(effectiveTime));
              relationship.setActive(true);
              relationship.setModuleId(Long
                  .valueOf(conceptMap.get("defaultModule").getTerminologyId()));
              relationship.setTerminology(terminology);
              relationship.setTerminologyVersion(terminologyVersion);
              relationship.setCharacteristicTypeId(Long.valueOf(conceptMap
                  .get("defaultCharacteristicType").getTerminologyId()));
              relationship.setModifierId(Long.valueOf(
                  conceptMap.get("defaultModifier").getTerminologyId()));
              ...
              relationship.setTypeId(
                  Long.valueOf(conceptMap.get(type).getTerminologyId()));
              ...
              log.info("    IGNORE rel to modifier");
            } else {
    ...
    public void addModifierClass() throws Exception {
      ...
      if (!conceptMap.containsKey(code)) {
        ...
        concept.setTerminologyVersion(terminologyVersion);
        concept.setDefaultPreferredName(labelChars.toString());
        log.info("  Add modifier concept " + concept.getTerminologyId() + " "
            + concept.getDefaultPreferredName());
    ...
    public void modifierHelper(String codeToModify) throws Exception {
      ...
      while (cmpCode.length() > 2) {
        log.info("    Determine if " + cmpCode + " has modifiers");
        ...
        if (classToModifierMap.containsKey(cmpCode)) {
          ...
          for (final String modifier : classToModifierMap.get(cmpCode)) {
            ...
            log.info("      Use modifier " + modifier + " for " + cmpCode);
            ...
            if (excludedModifiersToMatchedCodeMap.containsKey(modifier)
              ...
              log.info("      Exclude modifier " + modifier + " for "
              ...
              if (!overrideExclusion(codeToModify, modifier)) {
                ...
                log.info("      Override exclude modifier " + modifier + " for "
                    + codeToModify);
              }
            }
          }
        ...
        cmpCode = ClamlLoaderAlgorithm.this.chdParMap.get(cmpCode);
        if (cmpCode == null) {
          break;
        }
      }
      ...
      if (modifiersForCode.size() > 0) {
        ...
        for (final String modifiedByCode : modifiersForCode) {
          ...
          if (codeToModify.length() == 3 && modifiedByCode.endsWith("_4")
            ...
            log.info("        Apply modifier " + modifiedByCode + " to "
                + codeToModify);
          ...
          // any children
            ...
            log.info("        Creating placeholder concept "
            ...
            log.info("        SKIPPING modifier " + modifiedByCode + " for "
                + codeToModify);
          }
    ...
    private boolean overrideExclusion(String code, String modifier) {
      ...
      log.info("    CHECK OVERRIDE " + code + ", " + cmpCode + ", " + modifier);
    ...
    private boolean isDescendantCode(String desc, String anc) {
      ...
      while (ClamlLoaderAlgorithm.this.chdParMap.get(currentCode) != null) {
        final String parent =
            ClamlLoaderAlgorithm.this.chdParMap.get(currentCode);
    ...
    private Concept createChildConcept(String childCode, Concept parentConcept,
      ...
      log.info("        Creating concept " + childCode + " from "
      ...
      if (conceptMap.containsKey(childConcept.getTerminologyId()))
        throw new IllegalStateException(
            "ALERT2!  " + childConcept.getTerminologyId() + " already in map");
