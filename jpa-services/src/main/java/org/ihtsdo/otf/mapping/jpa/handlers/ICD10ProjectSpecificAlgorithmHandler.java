...
public class ICD10ProjectSpecificAlgorithmHandler extends
	...
	public MapAdviceList computeMapAdvice(MapRecord mapRecord, MapEntry mapEntry)
		...
		/*
		 * For any mapRelation value other than 447637006, Find the allowed
		 * project advice that matches (on string, case-insensitive) and return
		 * that value. Throw an exception if no corresponding advice is found.
		 */
		/*
		 * if (mapEntry.getMapRelation() != null &&
	...
	public boolean isTargetCodeValid(String terminologyId) throws Exception {
		...
		if (terminologyId.matches(".[0-9].")) {
			 ...
			 * (terminologyId.startsWith("Y")) { if (terminologyId.charAt(1) ==
			 * '1' || terminologyId.charAt(1) == '2') return true; if
			 * (terminologyId.charAt(1) == '3' && (terminology.)) return true;
			 */
			// otherwise, if 3-digit code has children, return false
			TreePositionList tpList = contentService.getTreePositions(
	...
	public void computeTargetTerminologyNotes(TreePositionList treePositionList)
		...
		Logger.getLogger(ICD10ProjectSpecificAlgorithmHandler.class).info(
				"  Asterisk to dagger relationship type found: "
						+ asteriskRefSetId);

		Logger.getLogger(ICD10ProjectSpecificAlgorithmHandler.class).info(
				"  Dagger to asterisk relationship type found: "
						+ daggerRefSetId);
	 ...
	 *            the content service
	 * @param asteriskRefSetId
	 *            the asterisk ref set id
	 * @param daggerRefSetId
	 ...
	 * #
	 * isPropagatedRecordForReleaseProcessing(org.ihtsdo.otf.mapping.model.MapRecord
	 * )
	 */
	@Override
