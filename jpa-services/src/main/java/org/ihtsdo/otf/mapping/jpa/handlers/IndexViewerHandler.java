...
public class IndexViewerHandler {
  ...
  private List<String> performSearch(String terminology,
    ...
    // configure
    final Path selectedDomainDir = Paths.get(indexesDir);
    ...
    // Open index
    Logger.getLogger(this.getClass()).debug("  Open directory reader");
    final Map<String, Analyzer> fieldAnalyzers = new HashMap<>();

    try (final FSDirectory dir = FSDirectory.open(selectedDomainDir);
      ...
      try {
        ...
        if (scoreDocs.length != 1) {
          throw new Exception("Unexpected number of results ("
              + scoreDocs.length + ") for link search " + linkStr);
        }
