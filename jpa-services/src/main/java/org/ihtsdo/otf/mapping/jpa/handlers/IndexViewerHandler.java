...
public class IndexViewerHandler {
  ...
  public SearchResultList findIndexEntries(String terminology,
    ...
    String subSearchField, String subSubSearchField, boolean allFlag)
      throws Exception {
		...
		mainSearchResults = performCodeSearch(terminology, terminologyVersion, domain, searchField);
		if (mainSearchResults.size() == 0) {
			mainSearchResults = performSearch(terminology, terminologyVersion, domain, searchField, startLevel,
					endLevel, null,
		...
		if (subSubSearchField == null || subSubSearchField.equals("undefined") || subSubSearchField.equals("")) {
			...
			startLevel = Integer.parseInt(config.getProperty("index.viewer.subSubSearchStartLevel"));
  ...
  private List<String> performSearch(String terminology,
	 ...
	 *             the exception
	 */
	private List<String> performCodeSearch(String terminology, String terminologyVersion, String domain, String code)
			throws Exception {
		...
		return performSearch(terminology, terminologyVersion, domain, code.contains("code:") ? code : "code:" + code, 0,
				2, null, false);
	 ...
	 *             the exception
	 */
	private Document getDocumentForLink(String terminology, String terminologyVersion, String domain, String linkStr)
			throws Exception {
    ...
    final PerFieldAnalyzerWrapper analyzer = new PerFieldAnalyzerWrapper(
		...
		try {
			...
			if (scoreDocs.length != 1) {
				throw new Exception(
						"Unexpected number of results (" + scoreDocs.length + ") for link search " + linkStr);
			}
	 ...
	 *             the exception
	 */
	public SearchResultList getIndexDomains(String terminology, String terminologyVersion) throws Exception {
	 ...
	 */
	@SuppressWarnings("static-method")
	public SearchResultList getIndexPagesForIndex(String terminology, String terminologyVersion, String index)
			throws Exception {
    ...
    for (int i = 0; i < documents.size(); i++) {
      ...
      if (d.get("code") != null) {
    	  for (String code : d.getValues("code")) {
    		  Logger.getLogger(getClass()).info("code found: " + code);
    		  htmlFragment += "&nbsp;" + code;
	      }
      }
