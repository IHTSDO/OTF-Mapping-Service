...
public class WorkflowLegacyPathHandler extends AbstractWorkflowPathHandler {
	...
	public WorkflowLegacyPathHandler() {
		...
		firstSpecialistEditingState
				.addWorkflowCombination(new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.PUBLISHED)));
		...
		firstSpecialistEditingState.addWorkflowCombination(
				new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.NEW)));
		firstSpecialistEditingState.addWorkflowCombination(new WorkflowStatusCombination(
				Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.EDITING_IN_PROGRESS)));

		trackingRecordStateToActionMap.put(firstSpecialistEditingState, new HashSet<>(
		...
		firstSpecialistConflictState.addWorkflowCombination(new WorkflowStatusCombination(
		...
		trackingRecordStateToActionMap.put(firstSpecialistConflictState,
				new HashSet<>(Arrays.asList(WorkflowAction.ASSIGN_FROM_INITIAL_RECORD, WorkflowAction.FINISH_EDITING,
		...
		secondSpecialistEditingState.addWorkflowCombination(new WorkflowStatusCombination(
				Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.NEW)));
		secondSpecialistEditingState
				.addWorkflowCombination(new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION,
						WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.EDITING_IN_PROGRESS)));
		firstSpecialistEditingState.addWorkflowCombination(new WorkflowStatusCombination(Arrays
				.asList(WorkflowStatus.REVISION, WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.CONFLICT_DETECTED)));
		...
		conflictDetectedState.addWorkflowCombination(new WorkflowStatusCombination(Arrays
				.asList(WorkflowStatus.REVISION, WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.CONFLICT_DETECTED)));
		...
		conflictEditingState = new WorkflowPathState("Lead Conflict Review Incomplet)");
		conflictEditingState.addWorkflowCombination(new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION,
				WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.CONFLICT_NEW)));
		conflictEditingState.addWorkflowCombination(
				new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.CONFLICT_DETECTED,
						WorkflowStatus.CONFLICT_DETECTED, WorkflowStatus.CONFLICT_IN_PROGRESS)));
		trackingRecordStateToActionMap.put(conflictEditingState, new HashSet<>(
				Arrays.asList(WorkflowAction.FINISH_EDITING, WorkflowAction.SAVE_FOR_LATER, WorkflowAction.UNASSIGN)));

		// STATE: Lead review (complete)
		conflictFinishedState = new WorkflowPathState("Lead Conflict Review Complete");
	...
	public ValidationResult validateTrackingRecordForActionAndUser(TrackingRecord tr, WorkflowAction action,
		...
		// Minimum role : Specialist
			...
			if (currentRecord == null && !action.equals(WorkflowAction.ASSIGN_FROM_SCRATCH)) {
				result.addError("Action is not permitted for this user and state");
			}
