...
public class Rf2SnapshotLoaderAlgorithm extends RootServiceJpa
   ...
   */
  public Rf2SnapshotLoaderAlgorithm(String terminology, String version,
  ...
  public void compute() throws Exception {
    ...
    try {
      ...
      log.info("      complete in " + getElapsedTime(startTime) + "s");
      ...
      if (conceptsByConcept != null) {
        ...
        log.info(String.format(elapsedTime, getElapsedTime(startTime),
            ft.format(new Date())));
      }
      ...
      if (descriptionsByDescription != null
        ...
        log.info("  Loading Descriptions and LanguageRefSets...");
        startTime = System.nanoTime();
        ...
        log.info("  Setting default preferred names for all concepts...");
        startTime = System.nanoTime();
      ...
      if (simpleRefsetsByConcept != null) {
        log.info("  Loading Simple RefSets...");
        startTime = System.nanoTime();
      ...
      if (simpleMapRefsetsByConcept != null) {
        log.info("  Loading SimpleMap RefSets...");
        startTime = System.nanoTime();
      ...
      if (extendedMapRefsetsByConcept != null) {
        log.info("  Loading ExtendedMap RefSets...");
        startTime = System.nanoTime();
      ...
      if (attributeRefsetsByDescription != null) {
        log.info("  Loading AttributeValue RefSets...");
        startTime = System.nanoTime();
  ...
  private void sortRf2Files(File coreInputDir, File outputDir)
    ...
    if (!outputDir.exists()
      ...
      // log reason for sort
      if (!outputDir.exists()) {
        log.info("     No sorted files exist -- sorting RF2 files");
      ...
      if (outputDir.exists()) {
        throw new Exception("Could not delete existing sorted files folder "
            + outputDir.toString());
      }

      // attempt to make sorted files directory
      if (outputDir.mkdir()) {
        log.info("  Creating new sorted files folder " + outputDir.toString());
      } else {
        throw new Exception("Could not create temporary sorted file folder "
            + outputDir.toString());
      }
    ...
    log.info("    Terminology dir = " + coreTerminologyInputDir.toString() + " "
    ...
    log.info("      Relationships file = " + coreRelInputFile.toString() + " "
    ...
    log.info("      Concepts file = " + coreConceptInputFile.toString() + " "
    ...
    log.info("      Descriptions file = " + coreDescriptionInputFile.toString()
    ...
    if (coreIdentifierInputFile != null) {
      log.info("      Identifiers file = " + coreIdentifierInputFile.toString()
    ...
    if (coreTextDefinitionInputFile != null) {
      log.info("      Text definitions file = "
    ...
    log.info("    Refset/Content dir = " + coreContentInputDir.toString() + " "
    ...
    for (File f : coreContentInputDir.listFiles()) {
      if (f.getName().contains("Association")) {
        if (coreAssociationReferenceInputFile != null)
          throw new Exception("Multiple Association Files!");
        coreAssociationReferenceInputFile = f;
      }
    }
    log.info("      Association file = "
    ...
    log.info("    Refset/Map dir = " + coreCrossmapInputDir.toString() + " "
    ...
    if (coreSimpleMapInputFile != null) {
      log.info("      Simple map file = " + coreSimpleMapInputFile.toString()
          + " " + coreSimpleMapInputFile.exists());
    }

    // Refset/Langauge dir
    File coreLanguageInputDir = new File(coreRefsetInputDir, "/Language/");
    log.info("    Refset/Language dir = " + coreLanguageInputDir.toString()
    ...
    log.info("      Language file = " + coreLanguageInputFile.toString() + " "
        + coreLanguageInputFile.exists());

    // Refset/Metadata dir
    File coreMetadataInputDir = new File(coreRefsetInputDir, "/Metadata/");
    log.info("    Refset/Metadata dir = " + coreMetadataInputDir.toString()
    ...
    if (coreAttributeValueInputFile != null) {
      sortRf2File(coreAttributeValueInputFile, attributeRefsetsByConceptFile,
          5);
    }
    // Sort simple file
    if (coreSimpleRefsetInputFile != null) {
  ...
  private void sortRf2File(File fileIn, File fileOut, final int sortColumn)
    ...
    log.info("        Sorting " + fileIn.toString() + "  into "
      ...
      if (!fields[0].equals("id")) {
        ...
        if (sourceConcept != null && destinationConcept != null) {
          ...
          if (sourceConcept == null) {
            log.info("Relationship " + relationship.getTerminologyId()
  ...
  private void loadDescriptionsAndLanguageRefSets(String terminology,
    ...
    while (!description.getTerminologyId().equals("-1")) {
      ...
      while (language.getDescription().getTerminologyId()
        ...
        log.info("     " + "Language Ref Set " + language.getTerminologyId()
      ...
      while (language.getDescription().getTerminologyId()
        ...
        /**if (description.isActive() && description.getTypeId().equals(dpnTypeId)
          ...
          if (defaultPreferredNames.get(concept.getId()) != null) {
            log.info("Multiple default preferred names for concept "
                + concept.getTerminologyId());
            log.info("  " + "Existing: "
                + defaultPreferredNames.get(concept.getId()));
            log.info("  " + "Replaced: " + description.getTerm());
          }
    ...
    log.info("      " + descCt + " descriptions loaded");
    log.info("      " + langCt + " language ref sets loaded");
    log.info("      " + skipCt + " language ref sets skipped (no description)");
    ...
    if ((line = descriptionsByDescription.readLine()) != null) {
      ...
      if (!fields[0].equals("id")) { // header
        ...
        if (concept != null) {
          ...
          log.info("Description " + description.getTerminologyId()
    ...
    while ((line = attributeRefsetsByDescription.readLine()) != null) {
      ...
      if (!fields[0].equals("id")) { // header
        ...
        if (concept != null) {
          ...
          log.debug("attributeValueRefSetMember "
    ...
    while ((line = simpleRefsetsByConcept.readLine()) != null) {
      ...
      if (!fields[0].equals("id")) { // header
        ...
        if (concept != null) {
          ...
          log.info("simpleRefSetMember " + simpleRefSetMember.getTerminologyId()
              + " references non-existent concept " + fields[5]);
        }
      }
    }
   ...
   * log.info("    commit = " + objectCt); contentService.commit();
   * contentService.beginTransaction(); } } else {
   ...
   * log.info("MEMORY USAGE:"); log.info(" Total: " + runtime.totalMemory());
   * log.info(" Free:  " + runtime.freeMemory()); log.info(" Max:   " +
   * runtime.maxMemory()); }
   */
