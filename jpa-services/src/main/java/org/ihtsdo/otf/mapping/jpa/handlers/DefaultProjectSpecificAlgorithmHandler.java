...
public class DefaultProjectSpecificAlgorithmHandler implements
   ...
   * 
   * @see
   * org.ihtsdo.otf.mapping.helpers.ProjectSpecificAlgorithmHandler#computeMapAdvice
  ...
  public ValidationResult validateTargetCodes(MapRecord mapRecord)
    ...
    /*
     * Group validation checks • Verify the last entry in a group is a TRUE rule
     ...
     * advice values are valid for the project (this can happen if “allowable
     * map advice” changes without updating map entries) • Entry must have
     * target code that is both in the target terminology and valid (e.g. leaf
     * nodes) OR have a relationId corresponding to a valid map category
		*/
  ...
  public ValidationResult checkMapRecordForDuplicateEntries(MapRecord mapRecord) {
    ...
    Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info(
        "  Checking map record for duplicate entries within map groups...");
		...
		for (int i = 0 ; i < entries.size() - 1; i++) {
          ...
          validationResult.addError("Duplicate entries found: " + "Group "
              + Integer.toString(entries.get(i).getMapGroup()) + ", priority "
              + Integer.toString(i) + " and " + "Group "
  ...
  public ValidationResult checkMapRecordNcNodes(MapRecord mapRecord,
    ...
    Logger
        ...
        .info(
            "  Checking map record for high-level groups with only NC target codes...");
    ...
    for (int i = 1; i < Math.max(groupToMapEntryList1.size(),
        ...
        validationResult
            .addError("Groups have different entry counts!  Record 1, Group "
                + i + ":" + entries1.size() + " Record 2, Group " + i + ":"
                + entries2.size());
	  	 }
      ...
      for (int d = 0; d < Math
        ...
        validationResult.addWarning("Group " + i
          ...
          if (entries1.get(d).getRule().equals(entries2.get(f).getRule())
              ...
              && !entries1.get(d).getMapRelation().getId()
                  .equals(entries2.get(f).getMapRelation().getId()))
	  	  		 matchFound = true;
	  	   }	  
		  	 if (matchFound) {
          validationResult
          ...
          validationResult
	  	 ...
	  	 for (int d=0; d<entries1.size(); d++) {
	  	   for (int f=0; f<entries2.size(); f++) {
		  	 ...
		  	 if (!matchFound) {
          validationResult.addError("Record "
              + convertToString(entries1.get(d))
  ...
  public WorkflowTrackingRecord assignFromInitialRecord(
    ...
    throws Exception {
        ...
        // set the owner and last modified by (timestamp automatically set in
  ...
  public WorkflowTrackingRecord assignFromScratch(
    ...
    throws Exception {
        ...
        // if a "new" tracking record (i.e. prior to conflict detection), add a
        // NEW record
        if (trackingRecord.getWorkflowStatus().compareTo(
          ...
          // otherwise, if this is a tracking record with conflict detected, add
          ...
          Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info(
              "NON_LEGACY_PATH: CONFLICT_IN_PROGRESS");
    ...
    // consuming
    // e.g. several minutes. If problematic, we could pass a "limit" parameter
    // and simply
  ...
  public WorkflowTrackingRecord unassign(WorkflowTrackingRecord trackingRecord,
    ...
    Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info(
        ...
        // determine action based on record's workflow status after removal of
      ...
      throw new Exception("finishEditing:  Record for user could not be found");
        ...
        if (trackingRecord.getWorkflowStatus().compareTo(
          ...
          List<MapRecord> mapRecords =
              new ArrayList<>(trackingRecord.getMapRecords());
				
				// check if two specialists have completed work
          if (trackingRecord.getLowestWorkflowStatus().equals(
            ...
            ValidationResult validationResult =
                compareMapRecords(mapRecords.get(0), mapRecords.get(1));
              ...
              Logger
                  .getLogger(DefaultProjectSpecificAlgorithmHandler.class)
                  .info(
                      "NON_LEGACY_PATH - No conflicts detected, ready for publication");
						
              // create a new record with default user and mark it with
              ...
              // conflict detected, change workflow status of all records and
              // update records
            ...
            Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class)
                .info("NON_LEGACY_PATH - Single record, updating");
				}
          ...
          Set<MapRecord> mapRecords =
              new HashSet<>(trackingRecord.getMapRecords());
              ...
              // set the CONFLICT_IN_PROGRESS record to READY_FOR_PUBLICATION
              // and update
					} else {
						mr.setWorkflowStatus(WorkflowStatus.READY_FOR_PUBLICATION);
					}
        ...
        Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info(
          ...
          // if the original PUBLISHED/READY_FOR_PUBLICATION record (i.e. now
          // has REVIEW), remove
				if (mr.getWorkflowStatus().equals(WorkflowStatus.REVIEW)) {
					trackingRecord.removeMapRecord(mr);
				} else {
