...
public class DefaultProjectSpecificAlgorithmHandler implements
  ...
  public ValidationResult compareMapRecords(MapRecord record1, MapRecord record2) {
    ...
    if (record1.isFlagForMapLeadReview()) {
      validationResult.addError("The first record requests MAP LEAD REVIEW.");
    }
    if (record2.isFlagForMapLeadReview()) {
      validationResult.addError("The second record requests MAP LEAD REVIEW.");
    }

    // check consensus review flag
    if (record1.isFlagForConsensusReview()) {
      validationResult.addError("The first record requests CONSENSUS REVIEW.");
    }
    if (record2.isFlagForConsensusReview()) {
      validationResult.addError("The second record requests CONSENSUS REVIEW.");
    }
    ...
    for (int i = 1; i < Math.max(groupToMapEntryList1.size(),
      ...
      if (entries1 == null) {
        validationResult.addError("The first record has an empty group " + i
            + ".");
        continue;
      } else if (entries2 == null) {
        validationResult.addError("The second record has an empty group " + i
            + ".");
        continue;
      ...
      if (outOfOrderFlag && !missingEntry) {
        validationResult
            .addWarning("The records have the same entries for group " + i
  ...
  public Set<MapRecord> assignFromScratch(TrackingRecord trackingRecord,
    ...
    // get the tree positions for this concept
        contentService
            .getTreePositionsWithDescendants(trackingRecord.getTerminologyId(),
  ...
  public Set<MapRecord> finishEditing(TrackingRecord trackingRecord,
    ...
    switch (trackingRecord.getWorkflowPath()) {
      ...
      case FIX_ERROR_PATH:
        ...
        // published record
        // requires a workflow state to exist below that of REVEW_NEW (in this
        ...
        if (mapRecords.size() == 2) {
          ...
          // Case 2: A lead has finished reviewing a corrected error and is
  ...
  public MapRecord getPreviouslyPublishedVersionOfMapRecord(MapRecord mapRecord)
    ...
    // cycle over records until the previously published/ready-for-publication
