...
public class DefaultProjectSpecificAlgorithmHandler implements
	...
	public ValidationResult compareMapRecords(MapRecord record1,
		...
		if (record1.isFlagForMapLeadReview()) {
			validationResult
					.addError("The first record requests MAP LEAD REVIEW.");
		}
		if (record2.isFlagForMapLeadReview()) {
			validationResult
					.addError("The second record requests MAP LEAD REVIEW.");
		}

		// check consensus review flag
		if (record1.isFlagForConsensusReview()) {
			validationResult
					.addError("The first record requests CONSENSUS REVIEW.");
		}
		if (record2.isFlagForConsensusReview()) {
			validationResult
					.addError("The second record requests CONSENSUS REVIEW.");
		}
		...
		for (int i = 1; i < Math.max(groupToMapEntryList1.size(),
			...
			if (entries1 == null) {
				validationResult
						.addError("The first record has an empty group " + i
								+ ".");
				continue;
			} else if (entries2 == null) {
				validationResult
						.addError("The second record has an empty group " + i
								+ ".");
				continue;
			} else if (entries1.size() != entries2.size()) {
				validationResult
						.addError("The records have different numbers of entries for group "
								+ i + ".");
			}
			...
			if (outOfOrderFlag && !missingEntry) {
				validationResult
						.addWarning("The records have the same entries for group "
								+ i + ", but in a different order.");
				continue; // to next group for comparison
			}
			...
			for (int d = 0; d < entries1.size(); d++) {
				...
				if (matchFound) {
					validationResult
							.addError("Records have a matching entry but with different map relation: "
			...
			for (int d = 0; d < entries1.size(); d++) {
				...
				if (matchFound) {
					validationResult
							.addError("Records have a matching entry but with different advice: "
									+ convertToString(entries1.get(d)) + ".");
				}
				matchFound = false;
			}
			for (int d = 0; d < entries1.size(); d++) {
				...
				if (matchFound) {
					validationResult
							.addError("Records have an entry with the same rule but different target code: "
									+ convertToString(entries1.get(d)) + ".");
				}
				matchFound = false;
			}
	...
	public Set<MapRecord> assignFromScratch(TrackingRecord trackingRecord,
		...
		case REVIEW_PROJECT_PATH:
			...
			Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info("Assigning concept along REVIEW_PROJECT_PATH");
		...
		*/		
				...
				if (mapRecords.size() != 0) {
					throw new Exception("Attempted to assign along the REVIEW_PROJECT_PATH where records already exist");
				}
	...
	public Set<MapRecord> finishEditing(TrackingRecord trackingRecord,
		...
		case REVIEW_PROJECT_PATH:
			...
			case NEW:
				...
				Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info("FinishEditing: REVIEW_PROJECT_PATH, Specialist level work");
				
				// check assumptions
				// - should only be one record
				if (mapRecords.size() != 1) {
					throw new Exception("FINISH called at initial editing level on REVIEW_PROJECT_PATH where more than one record exists");
				}
			...
			case REVIEW_NEW:
				...
				Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info("FinishEditing: REVIEW_PROJECT_PATH, Lead level work");
				
				
				// check assumptions
				// - should be two map records, this one and one marked REVIEW_NEEDED
				
				if (mapRecords.size() != 2)
					throw new Exception("FINISH called at review editing level on REVIEW_PROJECT_PATH without exactly two map records");
				...
				if (reviewRecord == null)
					throw new Exception("FINISH called at review editing level on REVIEW_PROJECT_PATH, but could not locate REVIEW_NEEDED record");
				
				// mark as READY_FOR_PUBLICATION
				mapRecord.setWorkflowStatus(WorkflowStatus.READY_FOR_PUBLICATION);
				
				// remove the specialist record
				newRecords.remove(reviewRecord);
				break;
		
			default:
				throw new Exception("Called finish on map record with invalid workflow status along REVIEW_PROJECT_PATH");
			}
		...
		case FIX_ERROR_PATH:
			...
			if (mapRecords.size() == 2) {
				...
				// Case 2: A lead has finished reviewing a corrected error and
				// is
				// ready to send it to publication status
