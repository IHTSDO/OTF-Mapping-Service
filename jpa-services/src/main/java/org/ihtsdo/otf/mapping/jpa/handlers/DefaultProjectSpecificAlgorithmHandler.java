...
public class DefaultProjectSpecificAlgorithmHandler implements
   ...
   */
  /*
   * @Override public Set<MapRecord> assignFromInitialRecord(TrackingRecord
   * trackingRecord, Set<MapRecord> mapRecords, MapRecord mapRecord, MapUser
   * mapUser) throws Exception {
   * 
   ...
   * 
   * // case 1 : User claims a PUBLISHED or READY_FOR_PUBLICATION record // to
   * // fix error on. if
   * (mapRecord.getWorkflowStatus().equals(WorkflowStatus.PUBLISHED) ||
   * mapRecord.getWorkflowStatus().equals(
   * WorkflowStatus.READY_FOR_PUBLICATION)) {
   * 
   * // check that only one record exists for this tracking record if
   * (!(trackingRecord.getMapRecordIds().size() == 1)) { throw new Exception(
   * "DefaultProjectSpecificHandlerException - assignFromInitialRecord: More than one record exists for FIX_ERROR_PATH assignment."
   * ); }
   * 
   * // deep copy the map record final MapRecord newRecord = new
   * MapRecordJpa(mapRecord, false);
   * 
   * // set origin ids newRecord.addOrigin(mapRecord.getId());
   * newRecord.addOrigins(mapRecord.getOriginIds());
   * 
   * // set other relevant fields newRecord.setOwner(mapUser);
   * newRecord.setLastModifiedBy(mapUser);
   * newRecord.setWorkflowStatus(WorkflowStatus.NEW);
   * 
   * // add the record to the list newRecords.add(newRecord);
   * 
   * // set the workflow status of the old record to review and add // it to //
   * new records mapRecord.setWorkflowStatus(WorkflowStatus.REVISION);
   * newRecords.add(mapRecord);
   * 
   * } break;
   * 
   * case LEGACY_PATH: break; case NON_LEGACY_PATH: throw new Exception(
   * "Invalid assignFromInitialRecord call for NON_LEGACY_PATH workflow"); case
   * REVIEW_PROJECT_PATH: throw new Exception(
   ...
   * newRecord.addOrigins(mapRecord.getOriginIds());
   * 
   * // set other relevant fields // get QA User MapUser
   * newRecord.setOwner(mapUser); newRecord.setLastModifiedBy(mapUser);
   ...
   */
  /**
   * @Override public Set<MapRecord> assignFromScratch(TrackingRecord
   *           trackingRecord, Set<MapRecord> mapRecords, Concept concept,
   *           MapUser mapUser) throws Exception {
   * 
   *           // the list of map records to return final Set<MapRecord>
   *           newRecords = new HashSet<>(mapRecords);
   * 
   *           // create new record final MapRecord mapRecord = new
   *           MapRecordJpa(); mapRecord.setMapProjectId(mapProject.getId());
   ...
   * 
   *           // set additional record parameters based on workflow path and
   *           status switch (trackingRecord.getWorkflowPath()) { case
   *           NON_LEGACY_PATH:
   * 
   *           // if a "new" tracking record (i.e. prior to conflict detection),
   *           // add a NEW record if (getWorkflowStatus(mapRecords).compareTo(
   *           WorkflowStatus.CONFLICT_DETECTED) < 0) {
   * 
   *           // check that this record is valid to be assigned (i.e. no more
   *           // than // one other specialist assigned) if
   *           (getMapUsers(mapRecords).size() >= 2) { throw new Exception(
   ...
   *           detected, add a CONFLICT_NEW record } else if
   ...
   *           ValidationResult validationResult = compareMapRecords(mapRecord1,
   *           mapRecord2);
   *           mapRecord.setReasonsForConflict(validationResult.getConciseErrors
   *           ());
   * 
   *           // get the origin ids from the tracking record for (final
   *           MapRecord mr : newRecords) { mapRecord.addOrigin(mr.getId()); }
   *           Logger
   *           .getLogger(DefaultProjectSpecificAlgorithmHandler.class).info(
   *           "NON_LEGACY_PATH: CONFLICT_NEW");
   * 
   *           } else { throw new
   *           Exception("ASSIGN_FROM_SCRATCH on NON_LEGACY_PATH failed."); }
   * 
   *           break;
   * 
   ...
   * 
   *           if (getWorkflowStatus(mapRecords).equals(WorkflowStatus.
   *           REVIEW_NEEDED)) { // check that one record exists and is not
   *           owned by this user if (mapRecords.size() == 1) {
   * 
   *           } else { throw new
   *           Exception("  Expected exactly one map record"); }
   * 
   ...
   *           mapRecord.setWorkflowStatus(WorkflowStatus.REVIEW_NEW); } else if
   *           (mapRecords.size() == 0) {
   * 
   ...
   *           case FIX_ERROR_PATH:
   * 
   *           // Case 1: A lead claims an error-fixed record for review if
   *           (getLowestWorkflowStatus(mapRecords).equals(
   *           WorkflowStatus.REVIEW_NEEDED)) {
   * 
   *           Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).
   *           info( "FIX_ERROR_PATH: Lead claiming an error-fixed record");
   * 
   *           // check that only two records exists for this tracking record if
   *           (!(trackingRecord.getMapRecordIds().size() == 2)) { throw new
   *           Exception(
   *           "assignFromScratch: There are not two records for FIX_ERROR_PATH assignment."
   *           ); }
   * 
   *           // set origin id for (final MapRecord mr : mapRecords) {
   ...
   * 
   * 
   *           case LEGACY_PATH: break; case QA_PATH:
   *           Logger.getLogger(DefaultProjectSpecificAlgorithmHandler
   *           .class).info( "Assigning concept along QA_PATH");
   * 
   *           if (getLowestWorkflowStatus(mapRecords)
   *           .equals(WorkflowStatus.QA_NEEDED)) { if (mapRecords.size() == 2)
   *           { // do nothing } else { throw new
   *           Exception("  Expected exactly two map records."); }
   * 
   *           // set origin id and copy labels for (final MapRecord record :
   *           mapRecords) { // if
   *           (record.getWorkflowStatus().equals(WorkflowStatus.REVISION)) //
   *           mapRecord.addOrigin(record.getId()); if
   *           (record.getWorkflowStatus().equals(WorkflowStatus.QA_NEEDED)) {
   *           mapRecord.setLabels(record.getLabels());
   *           mapRecord.addOrigin(record.getId()); } }
   * 
   *           // set workflow status to review new
   *           mapRecord.setWorkflowStatus(WorkflowStatus.QA_NEW); } else {
   *           throw new Exception(
   ...
   * 
   *           // For new or QA new, perform identification algorithm if
   *           (mapRecord.getWorkflowStatus().equals(WorkflowStatus.NEW) ||
   ...
   *           newRecords.add(mapRecord);
   * 
   *           // return the modified record set return newRecords; }
   */
