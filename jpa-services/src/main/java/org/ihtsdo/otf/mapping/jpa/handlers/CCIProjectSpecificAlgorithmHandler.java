...
public class CCIProjectSpecificAlgorithmHandler extends DefaultProjectSpecificAlgorithmHandler {
      ...
      if (concepts.keySet().size() == mapRecord.getMapEntries().size()) {
        ...
        for (int i = 0; i < mapRecord.getMapEntries().size(); i++) {
          ...
          if(entry.getMapRelation() == null) {
            result.addError("Entry " + entry.getMapGroup() + "/" + entry.getMapPriority() + " must be assigned a Relation.");
          }
        }
        
        //
        // PREDICATE: for map entries that have the relation "Partially classified",
        ...
        for (int i = 0; i < mapRecord.getMapEntries().size(); i++) {
          ...
          if (concept != null) {
            final MapEntry entry = mapRecord.getMapEntries().get(i);
            if(entry.getMapRelation() != null && entry.getMapRelation().getName().toLowerCase().contains("partially classified")) {
        ...
        //
        // PREDICATE: for map entries that don't have the relation "Partially classified",
        ...
        //
        // PREDICATE: an entry cannot have multiple Grades selected at the same time
        //
        for (int i = 0; i < mapRecord.getMapEntries().size(); i++) {
          final Concept concept = concepts.get(i + 1).get(0);
          if (concept != null) {
            ...
            if(entry.getMapRelation() != null && entry.getMapRelation().getName().toLowerCase().contains("partially classified")) {
              ...
              if(gradeCount > 1) {
                result
                .addError("Multiple grades selected for entry " + entry.getMapGroup() + "/" + entry.getMapPriority() + ".");
              }
            }
          }
        }
  ...
  public MapRecord computeInitialMapRecord(MapRecord mapRecord) throws Exception {
    ...
    try {
      ...
      MapRecordList finishedMapRecords = mappingService.getPublishedAndReadyForPublicationMapRecordsForMapProject(mapProject.getId(), new PfsParameterJpa());
      ...
      Concept mapRecordConcept = contentService.getConcept(mapRecord.getConceptId(), terminology, version);
      ...
      //If exactly 1 mapped parent identified, pre-assign CCI rubric level codes (5 digit) to child map record 
      if(parentMaps.size() == 1) {
        ...
        for(MapEntry mapEntry : existingMapRecord.getMapEntries()) {
          ...
          //Modify targetId to the 5 digit rubric level, and lookup the updated concept name
          final String modifiedTargetId = mapTargetId.substring(0, 7).concat(".^^");
          final Concept modifiedTargetConcept = contentService.getConcept(modifiedTargetId, mapProject.getDestinationTerminology(), mapProject.getDestinationTerminologyVersion());
          String modifiedTargetName = modifiedTargetConcept == null ? "CONCEPT NOT FOUND" : modifiedTargetConcept.getDefaultPreferredName();
      ...
      //If more than 1 mapped parent identified, create a note with map details
        ...
        for(MapRecord parentMapRecord : parentMaps) {
          ...
          noteStringBuilder.append("<br>Parent Map for " + parentMapRecord.getConceptId() + " - " + parentMapRecord.getConceptName() + " :<br>");
          noteStringBuilder.append("&nbsp;&nbsp;&nbsp;&nbsp;Map Entries<br>");
          for(MapEntry mapEntry : parentMapRecord.getMapEntries()) {
            noteStringBuilder
            .append("&nbsp;&nbsp;&nbsp;&nbsp;" + mapEntry.getMapGroup() + "/" + mapEntry.getMapPriority() + "   " + mapEntry.getTargetId() + " - " + mapEntry.getTargetName() + "<br>");
          }
  ...
  public boolean isTargetCodeValid(String terminologyId) throws Exception {
    ...
    try {
      ...
      // Valid concepts must contain a "." character.  Others are Section or Grouper concepts.
      if(!concept.getTerminologyId().contains(".")) {
        return false;
      }
      
      // Additionally, concepts must be at the Rubric level or below to be selectable.
      // Rubric level is defined as having codes for the first 5 characters, followed by ^^ wildcards.
      // e.g. 1.VP.87.^^ is a valid Rubric code, whereas 1.VP.^^.^^ is invalid.
      if(concept.getTerminologyId().length() >= 7 && concept.getTerminologyId().substring(5, 7).equals("^^")) {
        return false;
      }
