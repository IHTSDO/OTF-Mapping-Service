...
public class WorkflowFixErrorPathHandler extends AbstractWorkflowPathHandler {
	...
	public WorkflowFixErrorPathHandler() {
		...
		specialistFinishedState.addWorkflowCombination(
				new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.REVIEW_NEEDED)));
		...
		leadEditingState.addWorkflowCombination(new WorkflowStatusCombination(
				Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.REVIEW_NEEDED, WorkflowStatus.REVIEW_NEW)));
		leadEditingState.addWorkflowCombination(new WorkflowStatusCombination(Arrays.asList(WorkflowStatus.REVISION,
		...
		leadFinishedState.addWorkflowCombination(new WorkflowStatusCombination(
				Arrays.asList(WorkflowStatus.REVISION, WorkflowStatus.REVIEW_NEEDED, WorkflowStatus.REVIEW_RESOLVED)));
	...
	public ValidationResult validateTrackingRecordForActionAndUser(TrackingRecord tr, WorkflowAction action,
		...
		if (tr == null) {
			result.addError("Blank, non-committed tracking record required (i.e. project and concept specified only)");
		} else if (state == null) {
			...
			// record
				...
				if (!currentRecord.getWorkflowStatus().equals(WorkflowStatus.REVIEW_NEEDED)) {
					result.addError("User's record is marked " + currentRecord.getWorkflowStatus().toString()
		...
		if (result.getErrors().size() != 0) {
			result.addError("Error occured on " + getName() + " in workflow state "
					+ (state != null ? state.getWorkflowStateName() : "Undetermined State"));
		}
	...
	public SearchResultList findAssignedWork(MapProject mapProject, MapUser mapUser, MapUserRole userRole, String query,
		...
		Logger.getLogger(this.getClass()).info(getName() + ": findAssignedWork for project " + mapProject.getName()
				+ " and user " + mapUser.getUserName());
	...
	public Set<MapRecord> processWorkflowAction(TrackingRecord trackingRecord, WorkflowAction workflowAction,
		...
		case CANCEL:
			...
			if (newRecord == null)
				throw new Exception("Cancel work called for user " + mapUser.getName() + " and "
						+ trackingRecord.getTerminology() + " concept " + trackingRecord.getTerminologyId()
			...
			if (newRecord.getWorkflowStatus().equals(WorkflowStatus.NEW)) {
				...
				newRecords = processWorkflowAction(trackingRecord, WorkflowAction.UNASSIGN, mapProject, mapUser,
		...
		case FINISH_EDITING:
			...
			if (mapRecords.size() == 2) {
				...
				if (!foundModifiedRecord)
					throw new Exception("FIX_ERROR_PATH: Specialist finished work, but could not find their record");
		...
		case PUBLISH:
			...
			if (!revisionRecordFound)
				throw new Exception("Publish called on FIX_ERROR_PATH, but no REVISION record found");

			if (!reviewNeededRecordFound)
				throw new Exception("Publish called on FIX_ERROR_PATH, but no REVIEW_NEEDED record found");
			...
			Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class)
					.info("publish - FIX_ERROR_PATH - Creating READY_FOR_PUBLICATION record " + mapRecord.toString());
		...
		case SAVE_FOR_LATER:
			...
			break;
		case UNASSIGN:
			Logger.getLogger(DefaultProjectSpecificAlgorithmHandler.class).info("Unassign:  FIX_ERROR_PATH");
