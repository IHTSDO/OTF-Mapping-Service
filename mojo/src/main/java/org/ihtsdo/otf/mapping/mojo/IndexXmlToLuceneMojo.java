...
public class IndexXmlToLuceneMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException {
    ...
    try {
      ...
      // setting!
      String indexBaseDir = ConfigUtility.getConfigProperties()
          .getProperty("hibernate.search.default.indexBase");
      outputDir = indexBaseDir + "/" + terminology + "/" + terminologyVersion
          + "/" + domain;
      ...
      writer =
          new IndexWriter(new SimpleFSDirectory(Paths.get(outputDir)), config);
  ...
  class LocalHandler extends DefaultHandler {
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private void addDocumentForMainTerm()
      throws CorruptIndexException, IOException {
      Document doc = new Document();
      // getLog().info("mainTerm = " + data.get("title") + ", " +
      // data.get("code") + ", " + data.get("nemod") + ", " +
      // data.get("aname") + ", " + String.valueOf(hasChild));
      doc.add(new TextField("s", getFirstWord(data.get("title")), Store.NO));
      // , Index.ANALYZED, TermVector.NO));

      doc.add(new StringField("hasChild", String.valueOf(hasChild), Store.NO));
      // , Index.ANALYZED, TermVector.NO));
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private void addDocumentForTerm()
      throws CorruptIndexException, IOException {
