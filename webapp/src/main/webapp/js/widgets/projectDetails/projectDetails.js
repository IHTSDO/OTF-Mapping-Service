				...
				[
						...
						function($scope, $http, $sce, $rootScope, $location,
							...
							$scope.go = function() {
								 ...
								 * $scope.focusProject.mapPrincipleDocumentName = "Mapping
								 * Personnel Handbook"; } else {
								...
								for (var i = 0; i < $scope.focusProject.reportDefinition.length; i++) {
									if ($scope.focusProject.reportDefinition[i].qacheck == true)
										$scope.focusProject.qaCheckDefinition
												.push($scope.focusProject.reportDefinition[i]);
								}
							...
							$scope.getPagedAdvices = function(page, filter) {
								...
								$scope.pagedAdvice = $scope.sortByKey(
										$scope.focusProject.mapAdvice, 'name').filter(
							...
							$scope.getPagedRelations = function(page, filter) {
								...
								$scope.pagedRelation = $scope.sortByKey(
										$scope.focusProject.mapRelation, 'name').filter(
							...
							$scope.getPagedPrinciples = function(page, filter) {
								...
								$scope.pagedPrinciple = $scope.sortByKey(
										$scope.focusProject.mapPrinciple, 'principleId').filter(
							...
							$scope.getPagedReportDefinitions = function(page, filter) {
								...
								$scope.pagedReportDefinition = $scope.sortByKey(
										$scope.focusProject.reportDefinition, 'name').filter(
          ...
          for (var j = 0; j < $scope.pagedReportDefinition.length; j++) {
            if ($scope.pagedReportDefinition[j].qacheck == true) {
              $scope.pagedReportDefinition.splice(j, 1);
								...
								console.debug("pagedReportDefinition",
										$scope.pagedReportDefinition);
							};
							...
							$scope.getPagedQACheckDefinitions = function(page, filter) {
								...
								$scope.pagedQACheckDefinition = $scope.sortByKey(
										$scope.focusProject.qaCheckDefinition, 'name').filter(
								...
								console.debug("pagedQACheckDefinition",
										$scope.pagedQACheckDefinition);
							};
								...
								$http(
										{
											url : root_mapping + "project/id/"
													+ $scope.focusProject.id + "/scopeConcepts",
											dataType : "json",
							...
							$scope.getPagedScopeExcludedConcepts = function(page, filter) {
								...
								$http(
										{
											url : root_mapping + "project/id/"
													+ $scope.focusProject.id + "/scopeExcludedConcepts",
											dataType : "json",
											data : pfsParameterObj,
											method : "POST",
											headers : {
							...
							function containsPrincipleFilter(element) {
								...
								if (element.name.toString().toUpperCase().indexOf(
										$scope.principleFilter.toString().toUpperCase()) != -1)
									return true;
								if (element.sectionRef != null && element.sectionRef.toString().toUpperCase().indexOf(
										$scope.principleFilter.toString().toUpperCase()) != -1)
									return true;
							...
							function containsScopeExcludedConceptFilter(element) {
								...
								// filter matches upper-case element name or
								// detail
								if (element.scopeExcludesConceptId.toString().toUpperCase()
										.indexOf(
												$scope.scopeExcludesConceptFilter.toString()
														.toUpperCase()) != -1)
									return true;
								if (element.name.toString().toUpperCase().indexOf(
							...
							$scope.addMapUserToMapProjectWithRole = function(user, role) {
								...
								if (role != 'Specialist' && role != 'Lead') {
									console.debug("addNewMapUser called with invalid role: ",
											role);
									return;
								}
							...
							$scope.removeScopeIncludedConcept = function(scopeConcept,
									currentPage) {
								...
								$http(
										{
											url : root_mapping + "project/id/"
													+ $scope.focusProject.id + "/scopeConcept/remove",
							...
							$scope.removeScopeIncludedConcepts = function(
									scopeConceptsUnsplit) {
								...
								$http(
										{
											url : root_mapping + "project/id/"
													+ $scope.focusProject.id + "/scopeConcepts/remove",
											dataType : "json",
											data : scopeConcepts,
											method : "POST",
											headers : {
							...
							$scope.submitNewScopeIncludedConcepts = function(
									scopeConceptsUnsplit) {
								...
								$http(
										{
											url : root_mapping + "project/id/"
													+ $scope.focusProject.id + "/scopeConcepts/add",
											dataType : "json",
											data : scopeConcepts,
											method : "POST",
											headers : {
							...
							// remove a single concept (using the [x] button)
							$scope.removeScopeExcludedConcept = function(
									scopeExcludedConcept, currentPage) {
