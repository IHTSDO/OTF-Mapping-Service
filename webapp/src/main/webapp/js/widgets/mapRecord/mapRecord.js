...
angular
		...
		.controller(
				...
				function($scope, $rootScope, $http, $routeParams, $location,
						$sce, $modal, $window, localStorageService) {
					...
					$scope.saveGroups = function() {
						...
						for (var i = $scope.groupsTree.length - 1; i >= 0; i--) {
							...
							// if no entries, remove this group
							if ($scope.groupsTree[i].entry.length == 0) {
								$scope.removeGroup($scope.groupsTree[i]);
							} else {
					...
					function broadcastRecord() {
						...
						$rootScope.$broadcast(
								'mapRecordWidget.notification.recordChanged', {
					...
					$scope.resolveNextConcept = function(tooltipOnly) {
						...
						if ($scope.record.workflowStatus === 'NEW'
							...
							/*
							 * } else { $rootScope.glassPane--;
							 * console.debug("MapRecord finish/next can't
							 * determine type of work, returning to dashboard");
					...
					$scope.saveMapRecord = function(returnBack) {
						...
						console
								.debug("saveMapRecord called with "
										+ returnBack);
							...
							headers : {
								...
								.success(
										function(data) {
											...
											if (!returnBack) {
												console
														.debug("************* ReturnBack is false");

												$scope
														.resolveNextConcept(false);

											} else {
												console
														.debug("Simple save called, return to dashboard");
					...
					$scope.removeRecordPrinciple = function(record, principle) {
						record['mapPrinciple'] = removeJsonElement(
								record['mapPrinciple'], principle);
						$scope.record = record;
					...
					$scope.saveEditRecordNote = function(record, note) {
						...
						if ($scope.noteEditMode == true) {
							...
							if (noteFound = false) {
								console
										.debug("ERROR: Could not find note to edit with id = "
												+ $scope.noteEditId);
							}
						} else {
							console
					 ...
					 * FEEDBACK FUNCTIONS
					 */
					$scope.sendFeedback = function(record, feedbackMessage,
							recipientList) {
						...
						if (feedbackMessage == null
								...
								|| feedbackMessage === '') {
							window
									.alert("The feedback field cannot be blank. ");
							return;
						}
							...
							$http({
								...
								headers : {
									...
									.success(
											function(data) {
												console
														.debug("success to addFeedbackConversation.");
							...
							$http({
								...
								headers : {
									...
									.success(
											function(data) {
												console
														.debug("success to update Feedback conversation.");
											})
									.error(
											function(data, status, headers,
													config) {
						...
						if ($scope.project.groupStructure == false) {
							...
							for (var i = 0; i < $scope.entries.length; i++) {
								...
								for (var j = 0; j < $scope.entries[i].length; j++) {
									...
									console
											.debug("Assigning group and priority to "
													+ i + " " + j);
					...
					$scope.getRuleSummary = function(entry) {
						...
						if ($scope.project.ruleBased == true) {
							if (entry.rule.toUpperCase().indexOf("TRUE") != -1)
								...
								var lowerBound = entry.rule
										.match(/(>= \d+ [a-zA-Z]*)/);
								var upperBound = entry.rule
										.match(/(< \d+ [a-zA-Z]*)/);
					...
					$scope.selectMapEntry = function(entry) {
						...
						console.debug("Entry selected, new entries: ",
								$scope.entries);
					...
					$scope.deleteMapEntry = function(entry) {
						...
						$scope.groupsTree[entry.mapGroup].removeElement(entry);
					...
					Array.prototype.removeElement = function(elem) {
						...
						console.debug('before removeElement', elem,
								$scope.entries);

						// switch on type of id
						var idType = elem.hasOwnProperty('localId') ? 'localId'
								: 'id';
						...
						console.debug('after removeElement', elem,
								$scope.entries);
					};
					...
					$scope.openConceptBrowser = function() {
						var myWindow = window.open($scope.getBrowserUrl(),
								"browserWindow");
						myWindow.focus();
					};
