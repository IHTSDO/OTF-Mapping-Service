...
angular
    ...
    .controller(
        ...
        function($scope, $rootScope, $http, $location, $modal,
          ...
          $scope.retrieveAssignedQAWork = function(page, query,
              assignedWorkType) {
            ...
            $http(
                ...
                function(data) {
                  ...
                  for (var i = 0; i < $scope.assignedQAWork.length; i++) {
                    ...
                    $scope.assignedQAWork[i].labels = concept.value2.replace(
                        /;/g, ' ');
                  }
            ...
            $http(
                ...
                function(data) {
                  ...
                  $scope.numRecordPagesForUser = Math
                      .ceil($scope.nAssignedRecordsForUser
                          / $scope.itemsPerPage);
                    ...
                    function(data) {
                      ...
                      if ($scope.ownTab == true) {
                        ...
                        $scope.retrieveAssignedWork($scope.assignedWorkPage,
                            $scope.queryAssigned);
                        $scope.retrieveAssignedQAWork(
                            $scope.assignedQAWorkPage, $scope.queryQAWork);
                        if ($scope.currentRole === 'Lead'
                            || $scope.currentRole === 'Administrator') {
                          $scope.retrieveAssignedConflicts(
          ...
          // Unassigns all work (both concepts and conflicts) for the current
          // user
          $scope.unassignAllWork = function(unassignType, unassignEdited,
              mapUser) {

            $rootScope.glassPane++;

            var unassignUrlEnding = unassignEdited == true ? "/all"
                : "/unedited";
            ...
            $http(
                ...
                .success(
                    function(data) {
                      ...
                      if ($scope.ownTab == true) {
                        console
                            .debug('Viewing own work, retrieving assigned work');
                        $scope.retrieveAssignedWork($scope.assignedWorkPage);
                        if ($scope.currentRole === 'Lead'
                            || $scope.currentRole === 'Administrator') {
                          $scope
                              .retrieveAssignedConflicts($scope.assignedConflictsPage);
                        }
                        $rootScope
                            .$broadcast('assignedListWidget.notification.unassignWork');
                      } else {
          ...
          $scope.openUnassignModal = function(mapUser, workType) {
            ...
            var modalInstance = $modal
                .open({
                  ...
                  resolve : {
                    ...
                    // switch on whether Lead is viewing their own work or
                    // another
                    // user's
            ...
            // SearchResultList
            // that then can be used by either this function or a
            // display-affecting
            // function
            modalInstance.result
                .then(function(result) {
                  console.debug("Unassigning batch work for user "
                      + mapUser.userName + ", with parameters: ", result);

                  // assemble the list of things to unassign via filtered
                  // retrieval APIs
                  if (result.unassignConcepts == true) {
                    ...
                    $http(
                        ...
                        function(data) {
                          ...
                          console.debug("Unassigning concepts ("
                              + terminologyIdsConcepts.length + ")",
                              terminologyIdsConcepts);
                          unassignBatch(mapUser, terminologyIdsConcepts,
                              'concept');
                    ...
                    $http(
                        ...
                        function(data) {
                          ...
                          console.debug("Unassigning conflicts ("
                              ...
                              terminologyIdsConflicts);
                          unassignBatch(mapUser, terminologyIdsConflicts,
                              'conflict');
                        ...
                        .success(
                            function(data) {
                              ...
                              console.debug("Unassigning review work ("
                                  + terminologyIdsReview.length + ")",
                                  terminologyIdsReview);
                              unassignBatch(mapUser, terminologyIdsReview,
                                  'review');
                  ...
                  if (result.unassignQAWork == true) {
                    ...
                    $http(
                        ...
                        function(data) {
                          ...
                          // call the batch unassign API
                          console
                              .debug("Unassigning qa work ("
                                  + terminologyIdsQA.length + ")",
                                  terminologyIdsQA);
          ...
          /**
           * Function to open finish or publish modal. Argument: workflowStatus:
           * The filter by which to retrieve records Must be a valid workflow
           * status of type *_IN_PROGRESS or *_RESOLVED
           */
          $scope.finishOrPublishBatch = function(workflowStatus) {
            ...
            if (workflowStatus === 'CONFLICT_IN_PROGRESS'
              ...
              console
                  .debug("Could not determine api call from workflow status");
              return;
            }
            ...
            $http(
                {
                  url : root_workflow + "project/id/" + $scope.focusProject.id
                      ...
                      + "/query/null/" + apiWorkflowText, // set above based on
                                                          // specified workflow
                  // status
                  dataType : "json",
                  data : pfsParameterObj,
          ...
          $scope.openFinishOrPublishModal = function(records) {
            ...
            if (records == null || records.length == 0) {
              console
                  .error("openPerformBatchActionModal called with no records");
              return;
            }
            ;
          ...
          var FinishOrPublishWorkModalCtrl = function($scope, $modalInstance,
              $q, user, project, records, action) {
                          ...
                          headers : {
                            ...
                            .error(
                                function(data, status, headers, config) {
                                  ...
                                  console
                                      .debug("Failed to validate map record");
                                  $rootScope.handleHttpError(data, status,
                                      headers, config);
                                  deferred
                                      .reject("Map record failed validation");
                                });
            ...
            function finishAllRecordsHelper() {
              ...
              $scope.selectNextRecord().then(
                  function() {
                    ...
                    console.debug("Record selected, valid = " + $scope.validationResult.valid + ", finished = " + $scope.currentRecord.isFinished);

                    if ($scope.validationResult.valid == true
                        && $scope.currentRecord.isFinished == false) {

                      console
                          .debug("Finishing record...", $scope.currentRecord);
                      ...
                      // increment counter only if this record is not already finished
            ...
            $scope.finishAllRecords = function() {
              ...
              console.debug("Complete (finished/unfinished)", $scope.recordsFinished, $scope.recordsNotFinished);
