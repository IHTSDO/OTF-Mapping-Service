		...
		.controller(
				...
				[
						...
						function($scope, $http, $sce, $rootScope, $location,
							...
							$scope.currentRole = localStorageService
									.get('currentRole');
							$scope.currentUser = localStorageService
									.get('currentUser');
							$scope.focusProject = localStorageService
									.get('focusProject');
							$scope.mapProjects = localStorageService
									.get("mapProjects");
							$scope.mapUsers = localStorageService
									.get('mapUsers');
							...
							$scope.testReportError = null; // error returned
															// for report not
															// passing test
							$scope.definitionQueryTypes = [ 'SQL', 'HQL',
									'LUCENE' ];
							$scope.definitionResultTypes = [ 'CONCEPT',
									'MAP_RECORD' ];
							$scope.definitionRoles = [ 'VIEWER', 'SPECIALIST',
									'LEAD', 'ADMIN' ];
							$scope.definitionTimePeriods = [ 'DAILY', 'WEEKLY',
									'MONTHLY', 'ANNUALLY' ];
							...
							$scope.go = function() {
								$http(
										...
										.success(
												function(data) {
													console
															.debug("Success in getting terminologies metadata.");
							...
							$scope.getPagedAdvices = function(page, filter) {
								...
								$scope.pagedAdvice = $scope.sortByKey(
										$scope.mapAdvices, 'id').filter(
										containsAdviceFilter);
								$scope.pagedAdviceCount = $scope.pagedAdvice.length;
								$scope.pagedAdvice = $scope.pagedAdvice.slice(
										(page - 1) * $scope.pageSize, page
												* $scope.pageSize);
							...
							$scope.getPagedReportDefinitions = function(page,
									filter) {
								console.debug('getPagedReportDefinitions',
										filter);
								...
								$scope.pagedReportDefinition = $scope
										.sortByKey($scope.reportDefinitions,
												'id').filter(
							...
							$scope.getPagedQACheckDefinitions = function(page,
									filter) {
								console.debug('getPagedQACheckDefinitions',
										filter);
								if ($scope
										.qaCheckDefinitionInEditingPerformed() == true) {
									if (confirm("You have unsaved changes.\n\n Are you sure that you want to switch pages?") == false) {
								...
								$scope.pagedQACheckDefinition = $scope
										.sortByKey($scope.qaCheckDefinitions,
												'id')
							...
							function containsAdviceFilter(element) {
								...
								console.debug("Checking advice: ",
										$scope.adviceFilter);

								// check if advice filter is empty
								if ($scope.adviceFilter === ""
										|| $scope.adviceFilter == null)
									return true;
							...
							function containsReportDefinitionFilter(element) {
								...
								console.debug('checking name against filter',
										element.name)
							...
							function containsRelationFilter(element) {
								...
								console.debug("Checking relation: ",
										$scope.relationFilter);
								...
								// matches upper-case element name or detail
								if (element.terminologyId.toString()
										.toUpperCase().indexOf(
												$scope.relationFilter
														.toString()
														.toUpperCase()) != -1)
							...
							function containsPrincipleFilter(element) {
								...
								// detail
								if (element.principleId.toString()
										.toUpperCase().indexOf(
												$scope.principleFilter
														.toString()
														.toUpperCase()) != -1)
								...
								if (element.name.toString().toUpperCase()
										.indexOf(
												$scope.principleFilter
														...
														.toUpperCase()) != -1)
									return true;
								if (element.sectionRef.toString().toUpperCase()
										.indexOf(
												$scope.principleFilter
														.toString()
														.toUpperCase()) != -1)
							...
							$scope.deleteAdvice = function(advice) {
								console
										.debug("in deleteAdvice from application");
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to updateMapAdvice");
							...
							$scope.submitNewMapAdvice = function(mapAdviceName,
									mapAdviceDetail, allowableForNullTarget,
									isComputed) {
							...
							$scope.deleteRelation = function(relation) {
								console
										.debug("in deleteRelation from application");
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to updateMapRelation");
													removeComponentFromArray(
															editingPerformed,
															relation);
												})
							...
							$scope.submitNewMapRelation = function(
									mapRelationName, mapRelationAbbreviation,
									mapRelationTerminologyId,
									allowableForNullTarget, isComputed) {
								console
										.debug("in submitNewMapRelation for application");
								var obj = {
							...
							$scope.updatePrinciple = function(principle) {
								console
										.debug("in updatePrinciple for application");
								$http({
							...
							$scope.deletePrinciple = function(principle) {
								console
										.debug("in deletePrinciple from application");
							...
							$scope.submitNewMapPrinciple = function(
									mapPrincipleName, mapPrincipleId,
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to addMapPrinciple");
												})
										.error(
												function(data, status, headers,
														config) {
							...
							$scope.deleteAgeRange = function(ageRange) {
								...
								for (var j = 0; j < $scope.focusProject.mapAgeRange.length; j++) {
									if (ageRange.name === $scope.focusProject.mapAgeRange[j].name) {
										$scope.focusProject.mapAgeRange.splice(
												j, 1);
									}
								}
							...
							$scope.updateAgeRange = function(ageRange) {
								console
										.debug("in updateAgeRange for application");
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to updateAgeRange");
													removeComponentFromArray(
															editingPerformed,
															ageRange);
												})
							...
							$scope.submitNewMapAgeRange = function(name,
									lowerInclusive, lowerUnits, lowerValue,
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to addMapAgeRange");
							...
							$scope.getNonDiffReportDefinitionNames = function() {
								...
								if ($scope.reportDefinitions == null) return arr;
								
								for (var i = 0; i < $scope.reportDefinitions.length; i++) {
									console
											.debug($scope.reportDefinitions[i].diffReport);
									if ($scope.reportDefinitions[i].diffReport == false)
										arr
												.push($scope.reportDefinitions[i].name);
								}
								return arr;
							};
							...
							$scope.deleteReportDefinition = function(
									reportDefinition) {
								console
										.debug("in deleteReportDefinition from application");
							...
							$scope.validateReportDefinition = function(name,
									roleRequired, resultType, queryType,
									diffReport, timePeriod,
							...
							$scope.testReportDefinition = function(name,
									roleRequired, resultType, queryType,
									diffReport, timePeriod,
									diffReportDefinition, frequency, query) {
								...
								if ($scope.validateReportDefinition(name,
									roleRequired, resultType, queryType,
									diffReport, timePeriod,
							...
							$scope.updateReportDefinition = function(name,
									roleRequired, resultType, queryType,
									diffReport, timePeriod,
									diffReportDefinition, frequency, query) {
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to updateReportDefinition");
													removeComponentFromArray(
															editingPerformed,
															reportDefinition);
												})
							...
							$scope.submitNewReportDefinition = function(name,
									roleRequired, resultType, queryType,
									diffReport, timePeriod,
									diffReportDefinition, frequency, query) {
									...
									headers : {
										...
										.success(
												function(data) {
													...
													$scope.testReportError = null;
													console
															.debug("success to addReportDefinition");
												})
										.error(
												function(data, status, headers,
														config) {
							...
							$scope.deleteQACheckDefinition = function(
									qaCheckDefinition) {
								console
										.debug("in deleteQACheckDefinition from application");
							...
							$scope.updateQACheckDefinition = function(
									qaCheckDefinition) {
								...
								$http({
									...
									headers : {
										...
										.success(
												function(data) {
													console
															.debug("success to updateQACheckDefinition");
													removeComponentFromArray(
															editingPerformed,
															qaCheckDefinition);
												})
							...
							$scope.submitNewQACheckDefinition = function(name,
									roleRequired, resultType, queryType, query) {
								...
								$http(
										...
										.success(
												function(data) {
													console
															.debug("success to addQACheckDefinition");
												})
										.error(
												function(data, status, headers,
														config) {
							...
							$scope.submitNewMapProject = function(
									...
									newMapProjectDestinationVersion,
									newMapProjectRefSetId,
									newMapProjectPublished,
									newMapProjectRuleBased,
									newMapProjectGroupStructure,
									newMapProjectPublic,
									newMapProjectScopeDescendantsFlag,
									newMapProjectScopeExcludedDescendantsFlag,
									newMapProjectMapType,
									newWorkflowType,
									newMapRelationStyle,
									newHandler,
								...
								res = newMapProjectDestinationVersion
										.split(" ");
