    ...
    [
      ...
      function($scope, $http, $sce, $rootScope, $location, localStorageService,
        ...
        $scope.getPagedUsers = function(page, filter) {
          ...
          $scope.pagedUser = $scope.sortByKey($scope.mapUsers, 'id').filter(
            containsUserFilter);
        ...
        $scope.getPagedQaDefinitions = function(page, filter) {
          ...
          $scope.pagedQaDefinition = $scope.sortByKey(
            $scope.qaCheckDefinitions, 'id').filter(containsQaDefinitionFilter);
          $scope.pagedQaDefinitionCount = $scope.pagedQaDefinition.length;
          $scope.pagedQaDefinition = $scope.pagedQaDefinition.slice((page - 1)
            * $scope.pageSize, page * $scope.pageSize);
        ...
        function containsReportDefinitionFilter(element) {
          ...
          console
            .debug("Checking definitions: ", $scope.reportDefinitionFilter);
              ...
              if ($scope.mapProjectMetadata.keyValuePairList[i].name == 'Project Specific Handlers') {
                for (var j = 0; j < $scope.mapProjectMetadata.keyValuePairList[i].keyValuePair.length; j++) {
                  if ($scope.mapProjectMetadata.keyValuePairList[i].keyValuePair[j].key === 'default') {
                    ...
                    console
                      .debug("Found default project specific algorithm handler: "
                        + $scope.defaultProjectSpecificAlgorithmHandler);
                  }
                }
              }
            }
              ...
              if (data.qacheck == false) {
                for (var i = 0; i < $scope.pagedReportDefinition.length; i++) {
                  if ($scope.pagedReportDefinition[i].id == data.id) {
                    console.debug("Reverting definition:",
        ...
        $scope.deleteUser = function(user) {
          ...
          $http({
            ...
            console.debug("success to deleteMapUser from application");
          }).error(function(data, status, headers, config) {
        ...
        $scope.deleteQaDefinition = function(qaCheckDefinition) {
          ...
          $http({
            ...
            .success(function(data) {
              ...
              console.debug("success to deleteQaDefinition from application");
            })
            .error(
              function(data, status, headers, config) {
        ...
        $scope.submitNewMapProject = function(newMapProjectName,
          ...
          newMapProjectScopeExcludedDescendantsFlag, newMapProjectMapType,
          newWorkflowType, newMapRelationStyle, newHandler,
          ...
          if (newMapProjectPropagationFlag == true
            && newMapProjectPropagationThreshold == null) {
