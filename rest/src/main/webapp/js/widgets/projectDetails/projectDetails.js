      ...
      function($scope, $http, $sce, $rootScope, $location, localStorageService,
        ...
        $scope.getPagedAdvices = function(page, filter) {
          ...
          $scope.pagedAdvice = $scope.sortByKey($scope.focusProject.mapAdvice,
            'name').filter(containsAdviceFilter);
        ...
        $scope.addMapUserToMapProjectWithRole = function(user, role) {
          ...
          $http(
            {
              url : root_mapping + 'user/add?projectId='
                + $scope.focusProject.id + "&userRole=" + role.toUpperCase(),
              dataType : 'json',
              data : user,
              method : 'PUT',
              headers : {
            ...
            // update the project - this happens automatically now
            //$scope.updateMapProject();
        ...
        $scope.submitNewMapAdvice = function(mapAdviceName, mapAdviceDetail,
          ...
          $http(
            {
              url : root_mapping + 'advice/add?projectId='
        ...
        $scope.submitNewMapRelation = function(relation) {
          ...
          $http(
            {
              url : root_mapping + 'relation/add?projectId='
                + $scope.focusProject.id,
              dataType : 'json',
              data : relation,
              method : 'PUT',
        ...
        $scope.submitNewMapPrinciple = function(principle) {
          ...
          $http(
            {
              url : root_mapping + 'principle/add?projectId='
                + $scope.focusProject.id,
              dataType : 'json',
              data : principle,
              method : 'PUT',
        ...
        $scope.submitNewMapAgeRange = function(ageRange) {
          ...
          $http(
            {
              url : root_mapping + 'ageRange/add?projectId='
                + $scope.focusProject.id,
              dataType : 'json',
              data : ageRange,
              method : 'PUT',
        ...
        $scope.removeScopeIncludedConcept = function(scopeConcept, currentPage) {
          console
            .debug('removeScopeIncludedConcept', scopeConcept, currentPage);
          $rootScope.glassPane++;
        ...
        $scope.removeScopeExcludedConcept = function(scopeExcludedConcept,
          currentPage) {
          console.debug('removeScopeExcludedConcept', scopeExcludedConcept,
            currentPage);
          $rootScope.glassPane++;
        ...
        $scope.removeScopeExcludedConcepts = function(
          ...
          console.debug('removeScopeExcludedConcepts',
            scopeExcludedConceptsUnsplit);
          $rootScope.glassPane++;
        ...
        $scope.submitNewScopeExcludedConcepts = function(
          scopeExcludedConceptsUnsplit) {
          console.debug('submitNewScopeExcludedConcept',
            scopeExcludedConceptsUnsplit);
        ...
        $scope.processRelease = function() {
          ...
          if (!$scope.release.effectiveTime || !$scope.release.moduleId) {
            window
              .alert('Must set effective time and module id to process release');
          }

          // @Path("/project/id/{id:[0-9][0-9]*}/release/{effectiveTime}/module/id/{moduleId}/process")
          $http.post(
            root_mapping + 'project/id/' + $scope.focusProject.id + '/release/'
              + $scope.release.effectiveTime + '/module/id/'
              + $scope.release.moduleId + '/process').then(
        ...
        $scope.finishRelease = function(testMode) {
          ...
          if (!$scope.release.effectiveTime) {
            window
              .alert('Must set effective time to finish or preview release');
            $rootScope.glassPane--;
            return;
          }
          ...
          $http.post(
            root_mapping + 'project/id/' + $scope.focusProject.id + '/release/'
              + $scope.release.effectiveTime + '/finish'
              + (testMode ? '?test=true' : '')).then(
            // Success
            function(response) {
              $rootScope.glassPane--;
            },
