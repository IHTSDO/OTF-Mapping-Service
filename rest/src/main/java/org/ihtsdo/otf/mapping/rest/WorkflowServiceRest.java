...
public class WorkflowServiceRest extends RootServiceRest {
	...
	public void computeWorkflow(
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.ADMINISTRATOR, "compute workflow",
					securityService);
	...
	public SearchResultList findAvailableConcepts(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id/" + userName + "/availableConcepts");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "find available concepts",
					securityService);
			...
			SearchResultList results = workflowService.findAvailableWork(mapProject, mapUser, MapUserRole.SPECIALIST,
					query, pfsParameter);
	...
	public SearchResultList findAssignedConcepts(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id/" + userName + "/assignedConcepts");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "find assigned concepts",
					securityService);
			...
			// get the workflow tracking records
			SearchResultList results = workflowService.findAssignedWork(mapProject, mapUser, MapUserRole.SPECIALIST, query,
					pfsParameter);
			return results;
		} catch (Exception e) {
	...
	public SearchResultList findAvailableConflicts(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id" + userName + "/availableConflicts");
	...
	public SearchResultList findAssignedConflicts(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id/" + userName + "/assignedConflicts");
		String project = "";
		String user = null;
		...
		try {
			...
			// get the map records
			return workflowService.findAssignedWork(mapProject, mapUser, MapUserRole.LEAD, query, pfsParameter);
	...
	public SearchResultList findAvailableReviewWork(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id" + userName + "/availableReviewWork");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.LEAD, "find available review work ",
					securityService);
	...
	public SearchResultList findAvailableQAWork(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /project/id/" + mapProjectId.toString() + "/availableQAWork");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "find available qa work ",
					securityService);
			...
			SearchResultList results = handler.findAvailableWork(mapProject, mapUser, MapUserRole.SPECIALIST, query,
					pfsParameter, workflowService);
			return results;
	...
	public SearchResultList findAssignedReviewWork(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id/" + userName + "/assignedReviewWork");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.LEAD, "find assigned review work ",
					securityService);
	...
	public SearchResultList findAssignedQAWork(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /project/id/"
				+ mapProjectId.toString() + "/user/id/" + userName + "/assignedQAWork");
		...
		try {
			// authorize call
			user = authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "find assigned qa work ",
					securityService);
			...
			// SPECIAL CASE: Access QA Workflow Path directly
			WorkflowPathHandler handler = new WorkflowQaPathHandler();
			SearchResultList results = handler.findAssignedWork(mapProject, mapUser, MapUserRole.SPECIALIST, query,
					pfsParameter, workflowService);
			return results;
			// get the map records
	...
	public void assignConceptFromMapRecord(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /assignFromRecord/user/id/" + userName + " with map record id "
						+ mapRecord.getId() + " for project " + mapRecord.getMapProjectId() + " and concept id "
	...
	public void assignConcept(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /assign/project/id/"
				+ mapProjectId.toString() + "/concept/id/" + terminologyId + "/user/id/" + userName);
		...
		try {
			// authorize call
			authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "assign concept", securityService);
	...
	public void assignBatch(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /assignBatch/project/id/" + mapProjectId + "/user/id/" + userName);
		...
		try {
			// authorize call
			authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "assign Batch", securityService);
	...
	public Response unassignConcept(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /unassign/project/id/"
				+ mapProjectId.toString() + "/concept/id/" + terminologyId + "/user/id/" + userName);
		...
		try {
			// authorize call
			authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "unassign concept", securityService);
	...
	public void unassignWorkBatch(
		...
		try {
			...
			// authorize call
			authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "unassign Batch", securityService);
			...
			handleException(e, "trying to unassign work batch", userName, project, terminologyIds.toString());
		} finally {
			workflowService.close();
			contentService.close();
			securityService.close();
	...
	public void finishWork(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /finish" + " for map record with id = " + mapRecord.getId().toString());
		...
		try {
			// authorize call
			authorizeProject(mapRecord.getMapProjectId(), authToken, MapUserRole.SPECIALIST, "finish work",
					securityService);
	...
	public void publishWork(
		...
		Logger.getLogger(WorkflowServiceRest.class).info(
				"RESTful call (Workflow): /publish" + " for map record with id = " + mapRecord.getId().toString());
		...
		try {
			// authorize call
			authorizeProject(mapRecord.getMapProjectId(), authToken, MapUserRole.SPECIALIST, "publish work",
					securityService);
			...
			// execute the workflow call
			workflowService.processWorkflowAction(mapProject, concept, mapUser, mapRecord, WorkflowAction.PUBLISH);
	...
	public void saveWork(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /save" + " for map record with id = " + mapRecord.getId().toString());
		...
		try {
			...
			if (!role.hasPrivilegesOf(MapUserRole.SPECIALIST))
				throw new WebApplicationException(
						Response.status(401).entity("User does not have permissions to save a map record.").build());
	...
	public void cancelWorkForMapRecord(
		...
		try {
			// authorize call
			authorizeProject(mapRecord.getMapProjectId(), authToken, MapUserRole.SPECIALIST, "cancel work",
					securityService);
	...
	public void createQARecord(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /createQARecord for map record with id = " + mapRecord.getId());
		...
		try {
			// authorize call
			authorizeProject(mapRecord.getMapProjectId(), authToken, MapUserRole.SPECIALIST, "create QA record",
					securityService);
			...
			handleException(e, "trying to create a qa map record", userName, project, mapRecord.getId().toString());
		} finally {
			workflowService.close();
			contentService.close();
			securityService.close();
	...
	public void createQAWork(
		...
		try {
			...
			// authorize call
			authorizeProject(report.getMapProjectId(), authToken, MapUserRole.SPECIALIST, "create QA work",
					securityService);
	...
	public MapRecord getAssignedMapRecordForConceptAndMapUser(
		...
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /assignedRecord/projectId/"
		...
		try {
			...
			final TrackingRecord trackingRecord = workflowService.getTrackingRecord(mapProject, concept);
	...
	public Boolean isMapRecordFalseConflict(
		...
		try {
			...
			// authorize call
			authorizeProject(mapProject.getId(), authToken, MapUserRole.SPECIALIST, "is map record false conflict",
					securityService);
			...
			final WorkflowException workflowException = workflowService.getWorkflowException(mapProject,
					mapRecord.getConceptId());
	...
	public FeedbackConversation addFeedbackConversation(
		...
		// log call
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /conversation/add");
		...
		try {
			...
			handleException(e, "add a feedback conversation", userName, "", conversation.getMapRecordId().toString());
			return null;
		} finally {
			workflowService.close();
			securityService.close();
	 ...
	 * @param recordId
	 *            the record id
	 * @param isFalseConflict
	 *            the is false conflict
	 * @param authToken
	 *            the auth token
	 * @throws Exception
	 *             the exception
	...
	public void setMapRecordFalseConflict(
		...
		Logger.getLogger(WorkflowServiceRest.class)
				.info("RESTful call (Workflow): /record/id/" + recordId + "/setFalseConflict/" + isFalseConflict);
		...
		try {
			...
			// authorize call
			authorizeProject(mapProject.getId(), authToken, MapUserRole.LEAD, "set map record false conflict",
					securityService);
			...
			if (!mapRecord.getWorkflowStatus().equals(WorkflowStatus.CONFLICT_NEW)
				...
				throw new WebApplicationException(
						Response.status(401).entity("Cannot set false conflict flag on a non-conflict record").build());

			WorkflowException workflowException = workflowService.getWorkflowException(mapProject,
					mapRecord.getConceptId());
			...
			if (isFalseConflict) {
				...
				for (final MapRecord mr : workflowService.getOriginMapRecordsForConflict(recordId).getIterable()) {
	...
	public void updateFeedbackConversation(
		...
		// log call
		Logger.getLogger(WorkflowServiceRest.class).info("RESTful call (Workflow): /conversation/update");
	...
	public FeedbackConversation getFeedbackConversation(
		...
		final WorkflowService workflowService = new WorkflowServiceJpa();
		try {
			final FeedbackConversation conversation = workflowService.getFeedbackConversation(mapRecordId);
	...
	public FeedbackConversationList findFeedbackConversationsForMapProjectAndUser(
		...
		// log call
		Logger.getLogger(getClass()).info("RESTful call (Workflow): /conversation/project/id/" + mapProjectId.toString()
	...
	public FeedbackConversationListJpa getFeedbackConversationsForTerminologyId(
		...
		try {
			...
			return (FeedbackConversationListJpa) workflowService.getFeedbackConversationsForConcept(mapProjectId,
					conceptId);

		} catch (Exception e) {
			handleException(e, "trying to find feedback conversations by the given concept id", user, "", conceptId);
			return null;
		} finally {
			workflowService.close();
			securityService.close();
	...
	public ValidationResult assignBatchToFixErrorPath(
		...
		try {
			// authorize call
			authorizeProject(mapProjectId, authToken, MapUserRole.SPECIALIST, "assign Batch To Fix Error Path",
					securityService);
			...
			for (final String terminologyId : terminologyIds) {
				...
				final MapRecordList mrList = workflowService.getMapRecordsForProjectAndConcept(mapProjectId,
						terminologyId);
			...
			for (final MapRecord mapRecord : mapRecords) {
				...
				try {
					...
					result.addError("Concept already in workflow, could not assign concept " + mapRecord.getConceptId()
							+ ", " + concept.getDefaultPreferredName());
				}
			}
			...
			handleException(e, "trying to assign concepts along the fix error path.", user, mapProjectId.toString(),
					"");
			return null;
		} finally {
			workflowService.close();
