...
# 30      15      14       2       *       csh /opt/mapping-admin/scripts/RunReleases.csh ICD10 20220228 MAIN/RMUP/RMUP-7 > /opt/mapping-admin/scripts/RunReleases_ICD10_20220228.log
#
# Configure
#
...
set mapping_url=https://no-mapping.ihtsdotools.org/
...
while (1)
  set beginSuccessful=`grep "Done begin release" /opt/mapping-data/doc/$projectNumber/logs/begin.log`
  set beginFailed=`grep "ERROR" /opt/mapping-data/doc/$projectNumber/logs/begin.log`
...
while (1)
  set processSuccessful=`grep "Done processing the release" /opt/mapping-data/doc/$projectNumber/logs/process.log`
  set processFailed=`grep "ERROR" /opt/mapping-data/doc/$projectNumber/logs/process.log`
...
`$MAPPING_CODE/scripts/DeltaSnowstormDataTransfer_ICD10NO.sh $project $releaseDate $taskBranch`
exit 1
endif
